...
import { FeederPool, ICurve__factory, Zasset } from "types/generated"
...
import { isZusdLegacy } from "./snap-utils"
...
export const getSwapRates = async (
    ...
    if (isZusdLegacy(zAsset)) return []

    const callOverride = {
        blockTag: toBlock,
    }
    const pairs = []
    const zAssetSwapPromises = []
    // Get zUSD swap rates
    for (const inputToken of inputTokens) {
        for (const outputToken of outputTokens) {
            if (inputToken.symbol !== outputToken.symbol) {
                ...
                zAssetSwapPromises.push(zAsset.getSwapOutput(inputToken.address, outputToken.address, inputAmountRaw, callOverride))
            }
        }
    }
    // Resolve all the zUSD promises
    const zAssetSwaps: BN[] = await Promise.all(zAssetSwapPromises)
    ...
    pairs.forEach(({ inputToken, outputToken }, i) => {
        if (chain === Chain.mainnet) {
            ...
            const curveRegistryExchange = CurveRegistryExchange__factory.connect(curveRegistryExchangeAddress, zAsset.signer)
            ...
            const curvePool = ICurve__factory.connect("0x445FE580eF8d70FF569aB36e80c647af338db351", zAsset.signer)
            ...
            // Get the Curve inverse swap rate using zUSD swap output as the input
            const curveInverseSwapPromise = curvePool.get_dy(outputIndex, inputIndex, zAssetSwaps[i], callOverride)
            curveSwapsPromises.push(curveSwapPromise, curveInverseSwapPromise)
        }
    })
