...
export function isFeederPool(asset: Zasset | ZusdEth | ZusdLegacy | FeederPool): asset is FeederPool {
...
export function isZusdEth(asset: Zasset | ZusdEth | ZusdLegacy | FeederPool): asset is ZusdEth {
    return (asset as ZusdEth).surplus !== undefined
}
...
export function isZusdLegacy(asset: Zasset | ZusdEth | ZusdLegacy | FeederPool): asset is ZusdLegacy {
    return (asset as ZusdLegacy).getBasketManager !== undefined
}
...
export const snapConfig = async (asset: Zasset | ZusdEth | ZusdLegacy | FeederPool, toBlock: number): Promise<void> => {
    let ampData
    if (isZusdLegacy(asset)) return
    if (isZusdEth(asset)) {
...
export const getBasket = async (
    asset: Zasset | ZusdEth | ZusdLegacy | FeederPool,
    bAssetSymbols: string[],
    zAssetName = "zBTC",
    quantityFormatter: QuantityFormatter,
    toBlock: number,
    tvlConfig?: TvlConfig,
    exposedLogic?: ExposedZassetLogic,
): Promise<void> => {
    if (isZusdLegacy(asset)) return
    ...
    let zAssetSurplus = BN.from(0)
    if (isZusdEth(asset)) {
        zAssetSurplus = await asset.surplus({
            blockTag: toBlock,
        })
    } else if (!isFeederPool(asset)) {
        zAssetSurplus = (
    ...
    const zAssetSupply = await asset.totalSupply({
        blockTag: toBlock,
    })
    console.log(`Surplus    ${formatUnits(zAssetSurplus)}`)
    console.log(`${zAssetName}       ${quantityFormatter(zAssetSupply)}`)
    const zAssetTotal = zAssetSupply.add(zAssetSurplus)
    ...
    if (exposedLogic) {
        ...
        // Sum of base assets less zAsset total supply less zAsset surplus
        const bAssetZassetDiff = k.sub(zAssetTotal)
        const bAssetZassetDiffBasisPoints = bAssetZassetDiff.mul(10000).div(zAssetTotal)
        console.log(
            `Total ${zAssetName} ${formatUnits(zAssetTotal)} (${formatUnits(
                bAssetZassetDiff,
            )} ${bAssetZassetDiffBasisPoints}bps over-collateralised)`,
        )
    }
...
export const getBalances = async (
    zAsset: Zasset | ZusdEth | ZusdLegacy,
    ...
    const zAssetBalance = await zAsset.totalSupply({
        blockTag: toBlock,
    })
    console.log("\nHolders")
    ...
    for (const account of accounts) {
        const balance = await zAsset.balanceOf(account.address, {
            blockTag: toBlock,
        })
        console.log(`${account.name.padEnd(26)} ${quantityFormatter(balance)} ${balance.mul(100).div(zAssetBalance)}%`)
        balanceSum = balanceSum.add(balance)
        balances.push(balance)
    }
    const otherBalances = zAssetBalance.sub(balanceSum)
    console.log(`${"Other".padEnd(26)} ${quantityFormatter(otherBalances)} ${otherBalances.mul(100).div(zAssetBalance)}%`)

    const surplus = isZusdEth(zAsset)
    ...
    console.log(`Total                      ${quantityFormatter(zAssetBalance)}`)
...
export const getMints = async (
    bAssets: Token[],
    zAsset: Zasset | ZusdEth | ZusdLegacy | FeederPool,
    fromBlock: number,
    toBlock: number,
    quantityFormatter: QuantityFormatter,
): Promise<TxSummary> => {
    const filter = zAsset.filters.Minted(null, null, null, null, null)
    const logs = await zAsset.queryFilter(filter, fromBlock, toBlock)
    ...
    logs.forEach((log: any) => {
        ...
        // zAssetQuantity is for Zasset. output is for FeederPool
        const quantity = log.args.zAssetQuantity || log.args.output
...
export const getMultiMints = async (
    ...
    const filter = zAsset.filters.MintedMulti(null, null, null, null, null)
    const logs = await zAsset.queryFilter(filter, fromBlock, toBlock)
...
export const getSwaps = async (
    ...
    const filter = zAsset.filters.Swapped(null, null, null, null, null, null)
    const logs = await zAsset.queryFilter(filter, fromBlock, toBlock)
...
export const getRedemptions = async (
    ...
    const filter = zAsset.filters.Redeemed(null, null, null, null, null, null)
    const logs = await zAsset.queryFilter(filter, fromBlock, toBlock)
    ...
    logs.forEach((log: any) => {
        ...
        console.log(
            `${log.blockNumber} ${log.transactionHash} ${outputBasset.symbol.padEnd(4)} ${quantityFormatter(
                log.args.zAssetQuantity,
            )} ${quantityFormatter(log.args.scaledFee, 18, 8)}`,
        )
        total = total.add(log.args.zAssetQuantity)
...
export const getMultiRedemptions = async (
    ...
    const filter = zAsset.filters.RedeemedMulti(null, null, null, null, null, null)
    const logs = await zAsset.queryFilter(filter, fromBlock, toBlock)
    ...
    logs.forEach((log) => {
        ...
        console.log(
            `${log.blockNumber} ${log.transactionHash} ${quantityFormatter(log.args.zAssetQuantity)} ${quantityFormatter(
                log.args.scaledFee,
                18,
                8,
            )}`,
...
export const getLiquidatorInterest = async (
    ...
    const filter = savingsManager.filters.LiquidatorDeposited(zAsset.address, null)
...
export const getCollectedInterest = async (
    ...
    // Get MintedMulti events where the zAsset is the minter
    const filter = zAsset.filters.MintedMulti(zAsset.address, null, null, null, null)
    ...
    logs.forEach((log: any) => {
        ...
        // For zAssets:
        // - Trading fees = zAssetQuantity
        // - Platform fees = zAssetQuantity
        // For Feeder Pools:
        // - Trading fees = log.args.output
        // - Platform fees = sum of the input quantities as log.args.output is 0
        let quantity = BN.from(0)
        if (log.args.zAssetQuantity !== undefined) {
            quantity = log.args.zAssetQuantity
...
export const getAaveTokens = async (
    ...
    const zusdLiqData = await liquidator.liquidations(USDT.integrator, { blockTag: toBlock.blockNumber })
    console.log(`Min AAVE/USDC rate ${formatUnits(zusdLiqData.minReturn, USDC.decimals)}`)
