...
task("integration-aave-deploy", "Deploys an instance of AaveV2Integration contract")
    .addParam(
        "asset",
        "Symbol of the zAsset or Feeder Pool providing liquidity to the integration. eg zUSD, GUSD or alUSD",
        undefined,
        types.string,
    )
...
task("integration-paave-deploy", "Deploys zUSD and zBTC instances of PAaveIntegration")
    .addParam(
        "asset",
        "Symbol of the zAsset or Feeder Pool providing liquidity to the integration. eg zUSD, GUSD or alUSD",
        undefined,
    ...
    .addOptionalParam("assetType", "'address' for zAssets or 'feederPool' for Feeder Pools", "feederPool", types.string)
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        // or can be a zAsset Vault like zUSD and zBTC
        ...
        // TODO this only works for Feeder Pools. Need to get the list of bAssets from arg for zAssets
        ...
        const zAsset = Zasset__factory.connect(liquidityProviderAddress, deployer)

        for (const bAsset of bAssets) {
            const migrateData = zAsset.interface.encodeFunctionData("migrateBassets", [[bAsset.address], integration.address])
...
subtask("liquidator-create", "Creates a liquidation of a platform reward")
    .addParam("asset", "Symbol of the zAsset or Feeder Pool. eg zUSD, zBTC, alUSD, HBTC", undefined, types.string)
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const zAssetAddress = assetToken.feederPool ? ZERO_ADDRESS : assetToken.address
