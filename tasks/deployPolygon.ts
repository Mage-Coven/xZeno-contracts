...
import { ZassetLibraryAddresses } from "types/generated/factories/Zasset__factory"
import { deployContract, logTxDetails } from "./utils/deploy-utils"
import { getChain, getChainAddress, resolveAddress } from "./utils/networkAddressFactory"
import { getSigner } from "./utils/signerFactory"
import { PMTA, PzUSD, PWMATIC, tokens } from "./utils/tokens"
...
const deployZasset = async (
    deployer: Signer,
    linkedAddress: ZassetLibraryAddresses,
    nexus: Nexus,
    delayedProxyAdmin: DelayedProxyAdmin,
    recolFee = simpleToExactAmount(5, 13),
): Promise<Zasset> => {
    const zAssetFactory = new Zasset__factory(linkedAddress, deployer)
    const impl = await deployContract<Zasset>(zAssetFactory, "Zasset Impl", [nexus.address, recolFee])
    const proxy = await deployContract<AssetProxy>(new AssetProxy__factory(deployer), "Zasset Proxy", [
        impl.address,
        delayedProxyAdmin.address,
        "0x", // Passing zero bytes as we'll initialize the proxy contract later
    ])
    return zAssetFactory.attach(proxy.address)
}

const deployInterestBearingZasset = async (
    deployer: Signer,
    nexus: Nexus,
    zUsd: Zasset,
    unwrapper: Unwrapper,
...
const mint = async (sender: Signer, bAssets: DeployedBasset[], zAsset: Zasset, scaledMintQty: BN) => {
    ...
    for (const bAsset of bAssets) {
        ...
        const tx = await bAsset.bAssetContract.approve(zAsset.address, approval)
        const receiptApprove = await tx.wait()
        console.log(
            `Approved zAsset to transfer ${formatUnits(scaledMintQty)} ${bAsset.symbol} from ${senderAddress}. gas used ${
    ...
    // Mint
    const tx = await zAsset.mintMulti(
    ...
    // Log minted amount
    const zAssetAmount = formatUnits(await zAsset.totalSupply())
    console.log(`Minted ${zAssetAmount} zAssets from ${formatUnits(scaledMintQty)} units for each bAsset. gas used ${receiptMint.gasUsed}`)
}

const save = async (sender: Signer, zAsset: Zasset, izAsset: SavingsContract, scaledSaveQty: BN) => {
    console.log(`About to save ${formatUnits(scaledSaveQty)} zAssets`)
    await zAsset.approve(izAsset.address, scaledSaveQty)
    await izAsset["depositSavings(uint256)"](scaledSaveQty, { gasLimit: 8000000 })
    console.log(`Saved ${formatUnits(scaledSaveQty)} zAssets to interest bearing zAssets`)
}
...
task("deploy-polly", "Deploys zUSD & System to a Polygon network")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        // Deploy zAsset dependencies
        const zassetLogic = await deployContract<ZassetLogic>(new ZassetLogic__factory(signer), "ZassetLogic")
        const managerLib = await deployContract<ZassetManager>(new ZassetManager__factory(signer), "ZassetManager")
        const linkedAddress = {
            "contracts/zasset/ZassetLogic.sol:ZassetLogic": zassetLogic.address,
            "contracts/zasset/ZassetManager.sol:ZassetManager": managerLib.address,
        }

        // Deploy zUSD Zasset
        const zUsd = await deployZasset(signer, linkedAddress, nexus, delayedProxyAdmin)
        ...
        const izUsd = await deployInterestBearingZasset(
            signer,
            nexus,
            zUsd,
            unwrapper,
        ...
        // SaveWrapper contract approves the savings contract (izUSD) to spend its USD zAsset tokens (zUSD)
