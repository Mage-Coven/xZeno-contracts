...
task("feeder-snap", "Gets feeder transactions over a period of time")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const zAsset = tokens.find((t) => t.symbol === fAsset.parent)
        const fpAssets = [zAsset, fAsset]
...
task("feeder-rates", "Feeder rate comparison to Curve")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        // Get the bAssets for the main pool. eg bAssets in zUSD or zBTC
...
task("feeder-mint", "Mint some Feeder Pool tokens")
    .addOptionalParam("amount", "Amount of the zAsset and fAsset to deposit", undefined, types.float)
    .addParam("fasset", "Token symbol of the feeder pool asset. eg HBTC, GUSD, PFRAX or alUSD", undefined, types.string)
    .addOptionalParam("single", "Only mint using fasset. If false, does a multi mint using fasset and zasset", false, types.boolean)
    .addOptionalParam("speed", "Defender Relayer speed param: 'safeLow' | 'average' | 'fast' | 'fastest'", "fast", types.string)
    .setAction(async (taskArgs, hre) => {
        const signer = await getSigner(hre, taskArgs.speed)
        const chain = getChain(hre)
        ...
        const zAssetSymbol = feederPoolToken.parent
        if (!zAssetSymbol) throw Error(`No parent zAsset configured for feeder pool asset ${zAssetSymbol}`)
        const zAssetToken = tokens.find((t) => t.symbol === zAssetSymbol && t.chain === chain)
        if (!zAssetToken) throw Error(`Could not find zAsset token with symbol ${zAssetToken}`)
        ...
        if (taskArgs.single) {
            ...
            const tx = await fp.mintMulti([zAssetToken.address, feederPoolToken.address], [mintAmount, mintAmount], 0, signerAddress)
            await logTxDetails(
                tx,
                `Multi mint ${fpSymbol} from ${formatUnits(mintAmount)} ${zAssetSymbol} and ${formatUnits(mintAmount)} ${fAssetSymbol}`,
            )
        }
    })
...
task("feeder-swap", "Swap some Feeder Pool tokens")
    .addParam("input", "Token symbol of the input token to the swap. eg zUSD, zBTC, HBTC, GUSD, FRAX or alUSD", undefined, types.string)
    .addParam("output", "Token symbol of the output token from the swap. eg zUSD, zBTC, HBTC, GUSD, FRAX or alUSD", undefined, types.string)
