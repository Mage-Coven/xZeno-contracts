...
contract Liquidator is ILiquidator, Initializable, ModuleKeysStorage, ImmutableModule {
    ...
    ****************************************/
     ...
     * @param _zAsset optional address of the zAsset. eg zUSD or zBTC. Use zero address if from a Feeder Pool.
    ...
    function createLiquidation(
        ...
        if (_zAsset != address(0)) {
            // This Liquidator contract approves the zAsset to transfer bAssets for mint.
            // eg USDC in zUSD or WBTC in zBTC
            IERC20(_bAsset).safeApprove(_zAsset, 0);
            IERC20(_bAsset).safeApprove(_zAsset, type(uint256).max);

            // This Liquidator contract approves the Savings Manager to transfer zAssets
            ...
            IERC20(_zAsset).safeApprove(savings, 0);
            IERC20(_zAsset).safeApprove(savings, type(uint256).max);
        }
    ...
    function reApproveLiquidation(address _integration) external {
        ...
        address zAsset = liquidation.zAsset;
        if (zAsset != address(0)) {
            IERC20(bAsset).safeApprove(zAsset, 0);
            IERC20(bAsset).safeApprove(zAsset, type(uint256).max);

            address savings = _savingsManager();
            IERC20(zAsset).safeApprove(savings, 0);
            IERC20(zAsset).safeApprove(savings, type(uint256).max);
        }
    }
    ...
    ****************************************/
     ...
     *      - COMP for USDC
     *      - ALCX for alUSD
     *    - If bAsset in zAsset. eg USDC in zUSD
     *      - Mint zAsset using bAsset. eg mint zUSD using USDC.
     *      - Deposit zAsset to Savings Manager. eg deposit zUSD
    ...
    function triggerLiquidation(address _integration) external override {
        ...
        address zAsset = liquidation.zAsset;
        // If the integration contract is connected to a zAsset like zUSD or zBTC
        if (zAsset != address(0)) {
            // 4a. Mint zAsset using purchased bAsset
            uint256 minted = _mint(bAsset, zAsset);

            // 5a. Send to SavingsManager
            address savings = _savingsManager();
            ISavingsManager(savings).depositLiquidation(zAsset, minted);

            emit Liquidated(sellToken, zAsset, minted, bAsset);
        } else {
            ...
            emit Liquidated(sellToken, zAsset, bAssetBal, bAsset);
        }
    }
     ...
     *        - if from a zAsset
     *          - mints zAssets using bAssets. eg zUSD for USDC
     *          - deposits zAssets to Savings Manager. eg zUSD
    ...
    function triggerLiquidationAave() external override {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            // If the integration contract is connected to a zAsset like zUSD or zBTC
            if (zAsset != address(0)) {
                // 5a. Mint zAsset using bAsset from the Uniswap swap
                uint256 minted = _mint(bAsset, zAsset);

                // 6a. Send to SavingsManager to streamed to the savings vault. eg izUSD or izBTC
                address savings = _savingsManager();
                ISavingsManager(savings).depositLiquidation(zAsset, minted);

                emit Liquidated(aaveToken, zAsset, minted, bAsset);
            } else {
                ...
                emit Liquidated(aaveToken, zAsset, bAssetBal, bAsset);
            }
        }
    ...
    function _mint(address _bAsset, address _zAsset) internal returns (uint256 minted) {
        ...
        minted = IZasset(_zAsset).mint(_bAsset, bAssetBal, minOut, address(this));
    }
}
