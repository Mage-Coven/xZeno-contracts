...
import "../ZassetStructs.sol";

// Libs
import { SafeCast } from "@openzeppelin/contracts/utils/math/SafeCast.sol";
import { StableMath } from "../../shared/StableMath.sol";
import { ZassetManager } from "../ZassetManager.sol";
import { ZassetLogic } from "../ZassetLogic.sol";
import { MV1Migrator } from "./MV1Migrator.sol";

// Legacy
import { IBasketManager } from "../../z_mocks/zasset/migrate2/IBasketManager.sol";
import { Basket, Basset } from "../../z_mocks/zasset/migrate2/ZassetStructsV1.sol";
import { InitializableModuleV1 } from "../../z_mocks/zasset/migrate2/InitializableModuleV1.sol";

/**
 * @title   Zasset used to migrate zUSD from V2.0 to V3.0
 * @author  xZeno
 ...
 *          stablecoin (zAsset) and redirects lending market interest and swap fees to the savings
    ...
    ImmutableModule
{
     ...
     * @param _nameArg          Name of the zAsset
     * @param _symbolArg        Symbol of the zAsset
    ...
    ****************************************/
     ...
     * @param _input             Address of the bAsset to deposit for the minted zAsset.
     * @param _inputQuantity     Quantity in bAsset units
     * @param _minOutputQuantity Minimum zAsset quanity to be minted. This protects against slippage.
     * @param _recipient         Receipient of the newly minted zAsset tokens
     * @return mintOutput        Quantity of newly minted zAssets for the deposited bAsset.
     */
    function mint(
        address _input,
    ...
    /**
     * @dev Mint with multiple bAssets, at a 1:1 ratio to zAsset. This contract
     *      must have approval to spend the senders bAssets
     * @param _inputs            Non-duplicate address array of bASset addresses to deposit for the minted zAsset tokens.
     * @param _inputQuantities   Quantity of each bAsset to deposit for the minted zAsset.
     *                           Order of array should mirror the above bAsset addresses.
     * @param _minOutputQuantity Minimum zAsset quanity to be minted. This protects against slippage.
     * @param _recipient         Address to receive the newly minted zAsset tokens
     * @return mintOutput    Quantity of newly minted zAssets for the deposited bAssets.
     */
    function mintMulti(
        ...
        mintOutput = ZassetLogic.mintMulti(
            data,
            _getConfig(),
            indexes,
            _inputQuantities,
     ...
     * @param _input             Address of the bAsset to deposit for the minted zAsset
     * @param _inputQuantity     Quantity in bAsset units
     * @return mintOutput        Estimated mint output in zAsset terms
     */
        ...
        returns (uint256 mintOutput)
    {
        ...
        mintOutput = ZassetLogic.computeMint(
            data.bAssetData,
            input.idx,
            _inputQuantity,
            _getConfig()
    ...
    /**
     * @dev Get the projected output of a given mint
     * @param _inputs            Non-duplicate address array of addresses to bAssets to deposit for the minted zAsset tokens.
     * @param _inputQuantities  Quantity of each bAsset to deposit for the minted zAsset.
     * @return mintOutput        Estimated mint output in zAsset terms
     */
        ...
        returns (uint256 mintOutput)
    {
        ...
        uint8[] memory indexes = _getAssets(_inputs);
        return
            ZassetLogic.computeMintMulti(data.bAssetData, indexes, _inputQuantities, _getConfig());
    }
    ...
    ****************************************/
    ...
    function swap(
        ...
        uint256 scaledFee;
        (swapOutput, scaledFee) = ZassetLogic.swap(
            data,
            _getConfig(),
            input,
            output,
    ...
    function getSwapOutput(
        ...
        (swapOutput, ) = ZassetLogic.computeSwap(
            data.bAssetData,
            input.idx,
            output.idx,
            _inputQuantity,
    ...
    ****************************************/
    ...
    /**
     * @notice Redeems a specified quantity of zAsset in return for a bAsset specified by bAsset address.
     * The bAsset is sent to the specified recipient.
     * The bAsset quantity is relative to current vault balance levels and desired zAsset quantity.
     * The quantity of zAsset is burnt as payment.
     * A minimum quantity of bAsset is specified to protect against price slippage between the zAsset and bAsset.
     * @param _output            Address of the bAsset to receive
     * @param _zAssetQuantity    Quantity of zAsset to redeem
     * @param _minOutputQuantity Minimum bAsset quantity to receive for the burnt zAssets. This protects against slippage.
     * @param _recipient         Address to transfer the withdrawn bAssets to.
     * @return outputQuantity    Quanity of bAsset units received for the burnt zAssets
     */
    function redeem(
        address _output,
        uint256 _zAssetQuantity,
        uint256 _minOutputQuantity,
        address _recipient
    ) external override nonReentrant whenNoRecol returns (uint256 outputQuantity) {
        require(_recipient != address(0), "Invalid recipient");
        require(_zAssetQuantity > 0, "Qty==0");

        Asset memory output = _getAsset(_output);

        // Get config before burning. Config > Burn > CacheSize
        InvariantConfig memory config = _getConfig();
        _burn(msg.sender, _zAssetQuantity);

        uint256 scaledFee;
        (outputQuantity, scaledFee) = ZassetLogic.redeem(
            data,
            config,
            output,
            _zAssetQuantity,
     ...
     * balance levels and desired zAsset quantity. Burns the zAsset as payment.
     * @param _zAssetQuantity       Quantity of zAsset to redeem
     * @param _minOutputQuantities  Min units of output to receive
     * @param _recipient            Address to credit the withdrawn bAssets
     */
    function redeemZasset(
        uint256 _zAssetQuantity,
        ...
        uint256 scaledFee;
        (scaledFee, outputs, outputQuantities) = ZassetLogic.redeemProportionately(
            data,
            config,
            _zAssetQuantity,
     ...
     *      relative Zasset quantity from the sender. Sender also incurs a small fee on the outgoing asset.
     * @param _outputs           Addresses of the bAssets to receive
     * @param _outputQuantities  Units of the bAssets to redeem
     * @param _maxZassetQuantity Maximum zAsset quantity to burn for the received bAssets. This protects against slippage.
     * @param _recipient         Address to receive the withdrawn bAssets
     * @return zAssetQuantity    Quantity of zAsset units burned plus the swap fee to pay for the redeemed bAssets
     */
    function redeemExactBassets(
        address[] calldata _outputs,
        uint256[] calldata _outputQuantities,
        uint256 _maxZassetQuantity,
        address _recipient
    ) external override nonReentrant whenNoRecol returns (uint256 zAssetQuantity) {
        require(_recipient != address(0), "Invalid recipient");
        uint256 len = _outputQuantities.length;
        require(len > 0 && len == _outputs.length, "Invalid array input");
        require(_maxZassetQuantity > 0, "Qty==0");

        uint8[] memory indexes = _getAssets(_outputs);

        uint256 fee;
        (zAssetQuantity, fee) = ZassetLogic.redeemExactBassets(
            data,
            _getConfig(),
            indexes,
            _outputQuantities,
        ...
        _burn(msg.sender, zAssetQuantity);
     ...
     * @param _output            Address of the bAsset to receive
     * @param _zAssetQuantity    Quantity of zAsset to redeem
     * @return bAssetOutput      Estimated quantity of bAsset units received for the burnt zAssets
     */
    function getRedeemOutput(address _output, uint256 _zAssetQuantity)
        external
        view
        override
        returns (uint256 bAssetOutput)
    {
        ...
        (bAssetOutput, ) = ZassetLogic.computeRedeem(
            data.bAssetData,
            output.idx,
            _zAssetQuantity,
            _getConfig(),
     ...
     * @return zAssetQuantity    Estimated quantity of zAsset units needed to burn to receive output
     */
    function getRedeemExactBassetsOutput(
        address[] calldata _outputs,
        uint256[] calldata _outputQuantities
    ) external view override returns (uint256 zAssetQuantity) {
        uint256 len = _outputQuantities.length;
        require(len > 0 && len == _outputs.length, "Invalid array input");

        uint8[] memory indexes = _getAssets(_outputs);

        // calculate the value of zAssets need to cover the value of bAssets being redeemed
        (zAssetQuantity, ) = ZassetLogic.computeRedeemExact(
            data.bAssetData,
            indexes,
            _outputQuantities,
            _getConfig(),
    ...
    ****************************************/
    ...
    /**
     * @dev Get basket details for `Zasset_ZassetStructs.Basket`
    ...
    function getPrice() external view override returns (uint256 price, uint256 k) {
        return ZassetLogic.computePrice(data.bAssetData, _getConfig());
    }
    ...
    ****************************************/
    ...
    /**
     * @dev Converts recently accrued swap and redeem fees into zAsset
     * @return mintAmount   zAsset units generated from swap and redeem fees
     * @return newSupply    zAsset total supply after mint
     */
        ...
        returns (uint256 mintAmount, uint256 newSupply)
    {
        ...
        if (surplusFees > 1) {
            ...
            // mint new zAsset to savings manager
     ...
     *      amount of zAsset and sends it over to the SavingsZassetManager.
     * @return mintAmount   zAsset units generated from interest collected from lending markets
     * @return newSupply    zAsset total supply after mint
     */
    function collectPlatformInterest()
        ...
        returns (uint256 mintAmount, uint256 newSupply)
    {
        (uint8[] memory idxs, uint256[] memory gains) = ZassetManager.collectPlatformInterest(
            data.bAssetPersonal,
            data.bAssetData
        );

        mintAmount = ZassetLogic.computeMintMulti(data.bAssetData, idxs, gains, _getConfig());
    ...
    ****************************************/
     ...
     * @param _cacheSize Maximum percent of total zAsset supply to hold for each bAsset
     */
    ...
    function setTransferFeesFlag(address _bAsset, bool _flag) external override onlyGovernor {
        ZassetManager.setTransferFeesFlag(data.bAssetPersonal, bAssetIndexes, _bAsset, _flag);
    }
    ...
    function migrateBassets(address[] calldata _bAssets, address _newIntegration)
        ...
        onlyGovernor
    {
        ZassetManager.migrateBassets(data.bAssetPersonal, bAssetIndexes, _bAssets, _newIntegration);
    }
    ...
    function negateIsolation(address _bAsset) external onlyGovernor {
        ZassetManager.negateIsolation(data.basket, data.bAssetPersonal, bAssetIndexes, _bAsset);
    }
    ...
    function startRampA(uint256 _targetA, uint256 _rampEndTime) external onlyGovernor {
        ZassetManager.startRampA(data.ampData, _targetA, _rampEndTime, _getA(), A_PRECISION);
    }
    ...
    function stopRampA() external onlyGovernor {
        ZassetManager.stopRampA(data.ampData, _getA());
    }
    ...
    function mintDeficit() external returns (uint256 mintAmount) {
        ...
        InvariantConfig memory config = _getConfig();
        (, uint256 k) = ZassetLogic.computePrice(data.bAssetData, config);
