 ...
 * @title   FeederPool
 * @author  xZeno
 * @notice  Base contract for Feeder Pools (fPools). Feeder Pools are combined of 50/50 fAsset and zAsset. This supports
 *          efficient swaps into and out of zAssets and the bAssets in the zAsset basket (a.k.a mpAssets). There is 0
 *          fee to trade from fAsset into zAsset, providing low cost on-ramps into zAssets.
 * @dev     VERSION: 1.0
 *          DATE:    2021-03-01
 */
    ...
    InitializableReentrancyGuard
{
    ...
    address public immutable override zAsset;
     ...
     * @param _zAsset  Immutable zAsset address
     */
    constructor(address _nexus, address _zAsset) PausableModule(_nexus) {
        zAsset = _zAsset;
    }

    /**
     * @dev Basic initializer. Sets up core state and importantly provides infinite approvals to the zAsset pool
     * to support the cross pool swaps. bAssetData and bAssetPersonal are always ordered [zAsset, fAsset].
     * @param _nameArg     Name of the fPool token (a.k.a. fpToken)
     * @param _symbolArg   Symbol of the fPool token
     * @param _zAsset      Details on the base zAsset
     * @param _fAsset      Details on the attached fAsset
     * @param _mpAssets    Array of bAssets from the zAsset (to approve)
     * @param _config      Starting invariant config
     */
    function initialize(
        string calldata _nameArg,
        string calldata _symbolArg,
        BassetPersonal calldata _zAsset,
        ...
        require(_zAsset.addr == zAsset, "zAsset incorrect");
        data.bAssetPersonal.push(
            BassetPersonal(_zAsset.addr, _zAsset.integrator, false, BassetStatus.Normal)
        );
        ...
        for (uint256 i = 0; i < _mpAssets.length; i++) {
            ...
            IZasset(_zAsset.addr).getBasset(_mpAssets[i]);
            IERC20(_mpAssets[i]).safeApprove(_zAsset.addr, 2**255);
        }
    ...
    ****************************************/
     ...
     * Supports either fAsset, zAsset or mpAsset as input - with mpAssets used to mint zAsset before depositing.
     ...
     * Supports only fAsset or zAsset as inputs.
        ...
        returns (uint256 mintOutput)
    {
        ...
        if (input.exists) {
            ...
            uint256 estimatedZasset = IZasset(zAsset).getMintOutput(_input, _inputQuantity);
            mintOutput = FeederLogic.computeMint(data.bAssetData, 0, estimatedZasset, _getConfig());
        }
    }
    ...
    ****************************************/
    ...
    function getSwapOutput(
        ...
        // Internal swap between fAsset and zAsset
        ...
        if (input.exists) {
            // Swap into zAsset > Redeem into mpAsset
            (swapOutput, ) = FeederLogic.computeSwap(
                data.bAssetData,
                1,
                0,
            ...
            swapOutput = IZasset(zAsset).getRedeemOutput(_output, swapOutput);
        }
        // Else we are swapping into fAsset
        else {
            // Mint zAsset from mp > Swap into fAsset here
            swapOutput = IZasset(zAsset).getMintOutput(_input, _inputQuantity);
            (swapOutput, ) = FeederLogic.computeSwap(
                data.bAssetData,
                0,
                1,
    ...
    /**
     * @dev Checks if a given swap path is valid. Only fAsset<>zAsset & fAsset<>mpAsset swaps are supported.
     */
        ...
        returns (bool isValid)
    {
        ...
        // Path is into or out of zAsset - just use main pool for this
        return false;
    }
    ...
    ****************************************/
     ...
     * balance levels and desired fpToken quantity. Burns the fpToken as payment. Only fAsset & zAsset are supported in this path.
     ...
     *      relative fpToken quantity from the sender. Only fAsset & zAsset (0,1) are supported in this path.
        ...
        returns (uint256 bAssetOutput)
    {
        ...
        if (!output.exists) {
            bAssetOutput = IZasset(zAsset).getRedeemOutput(output.addr, bAssetOutput);
        }
    }
    ...
    function getRedeemExactBassetsOutput(
        ...
        uint256 zAssetRedeemed = FeederLogic.computeRedeemExact(
            data.bAssetData,
            indexes,
            _outputQuantities,
            _getConfig()
        );
        fpTokenQuantity = zAssetRedeemed.divPrecisely(1e18 - data.redemptionFee);
    ...
    ****************************************/
    ...
    function _getAsset(address _asset) internal view returns (Asset memory status) {
        // if input is zAsset then we know the position
        if (_asset == zAsset) return Asset(0, _asset, true);
