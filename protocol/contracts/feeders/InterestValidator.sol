...
contract InterestValidator is PausableModule {
    ...
    event GovFeeCollected(address indexed feederPool, address zAsset, uint256 amount);
     ...
     * zAsset and sends directly to the SavingsManager as unallocated interest.
     */
    function collectGovFees(address[] calldata _fPools) external onlyGovernor {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            // 2. If fpTokenBal > 0, convert to zAsset and transfer to savingsManager
            if (fpTokenBal > 0) {
                address zAsset = IFeederPool(fPool).zAsset();
                ...
                emit GovFeeCollected(fPool, zAsset, outputAmt);
            }
        }
    }
}
