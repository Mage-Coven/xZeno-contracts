...
library FeederLogic {
    ...
    ****************************************/
    ...
    function swap(
        ...
        // 1. [f/zAsset ->][ f/zAsset]               : Y - normal in, SWAP, normal out
        // 3. [mpAsset -> zAsset][ -> fAsset]        : Y - mint in  , SWAP, normal out
        if (_output.exists) {
        ...
        // 2. [fAsset ->][ zAsset][ -> mpAsset]      : Y - normal in, SWAP, mpOut
        else {
            address zAsset = _data.bAssetPersonal[0].addr;
            (swapOutput, localFee) = _swapLocal(
                _data,
                _config,
                cachedBassetData,
            ...
            swapOutput = IZasset(zAsset).redeem(
                _output.addr,
                swapOutput,
                _minOutputQuantity,
                _recipient
    ...
    ****************************************/
    ...
    function redeem(
        ...
        if (_output.exists) {
            ...
            outputQuantity = IZasset(zAsset).redeem(
                _output.addr,
                outputQuantity,
                _minOutputQuantity,
                _recipient
    ...
    function redeemExactBassets(
        ...
        require(fpTokenQuantity > 0, "Must redeem some zAssets");
        fpTokenQuantity += 1;
        require(fpTokenQuantity <= _maxInputQuantity, "Redeem zAsset qty > max quantity");

        // Burn the full amount of Zasset
    ...
    ****************************************/
    ...
    /**
     * @dev Transfers an asset in and updates vault balance. Supports fAsset, zAsset and mpAsset.
    ...
    /**
     * @dev Mints an asset in the main zAsset pool. Input asset must be supported by the zAsset
    ...
    function _mpMint(
        ...
        uint256 _inputQuantity,
        uint256 _maxCache
    ) internal returns (AssetData memory zAssetData) {
        zAssetData = AssetData(0, 0, _data.bAssetPersonal[0]);
        IERC20(_input.addr).safeTransferFrom(msg.sender, address(this), _inputQuantity);

        address integrator = zAssetData.personal.integrator == address(0)
            ? address(this)
            : zAssetData.personal.integrator;

        uint256 balBefore = IERC20(zAssetData.personal.addr).balanceOf(integrator);
        // Mint will revert if the _input.addr is not whitelisted on that zAsset
        IZasset(zAssetData.personal.addr).mint(_input.addr, _inputQuantity, 0, integrator);
        uint256 balAfter = IERC20(zAssetData.personal.addr).balanceOf(integrator);
        zAssetData.amt = balAfter - balBefore;

        // Route the zAsset to platform integration
        if (integrator != address(this)) {
            if (balAfter > _maxCache) {
                uint256 delta = balAfter - (_maxCache / 2);
                IPlatformIntegration(integrator).deposit(zAssetData.personal.addr, delta, false);
            }
        }
    }

    /**
     * @dev Performs a swap between fAsset and zAsset. If the output is an zAsset, do not
     * charge the swap fee.
     */
    function _swapLocal(
        FeederData storage _data,
    ...
    /**
     * @dev Performs a local redemption into either fAsset or zAsset.
     */
