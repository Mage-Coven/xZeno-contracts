 ...
 * @author  xZeno
 * @notice  Simply receives zAssets and then deposits to a pre-defined Balancer
 *          Bpool.
 * @dev     VERSION: 2.0
 *          DATE:    2021-04-06
 */
contract RevenueRecipient is IRevenueRecipient, ImmutableModule {
    ...
    event RevenueDeposited(address indexed zAsset, uint256 amountIn, uint256 amountOut);
     ...
     * @param _assets     Initial list of supported zAssets
     * @param _minOut     Minimum BPT out per zAsset unit
     */
    constructor(
        address _nexus,
        address _targetPool,
     ...
     * @param _zAssets Addresses of assets to deposit
        ...
        override
    {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            address zAsset = _zAssets[i];
            uint256 bal = IERC20(zAsset).balanceOf(address(this));
            // e.g. 1 * 5e17 / 1e18 = 5e17
            uint256 deposit = (bal * pct) / 1e18;
            require(minOut[zAsset] > 0, "Invalid minout");
            uint256 minBPT = (deposit * minOut[zAsset]) / 1e18;
            uint256 poolAmountOut = mBPT.joinswapExternAmountIn(zAsset, deposit, minBPT);

            emit RevenueDeposited(zAsset, deposit, poolAmountOut);
        }
    }
     ...
     * @param _asset Address of zAsset
     * @param _minOut Scaled amount to receive per 1e18 zAsset units
     */
