...
describe("RevenueRecipient", () => {
    ...
    const runSetup = async (): Promise<void> => {
        mXYZ = await zAssetMachine.loadBassetProxy("xZeno XYZ", "mXYZ", 18)
        BAL = await zAssetMachine.loadBassetProxy("Balance Gov Token", "BAL", 18)
    ...
    describe("creating new instance", () => {
        ...
        it("should give bPool permission to spend zAssets", async () => {
    ...
    describe("depositing revenue to pool", () => {
        ...
        describe("should fail if", () => {
            ...
            it("asset does not have minOut", async () => {
                const mZZZ = await zAssetMachine.loadBassetProxy("xZeno ZZZ", "mZZZ", 18)
                await expect(revenueRecipient.depositToPool([mZZZ.address], [simpleToExactAmount(8, 17)])).to.be.revertedWith(
                    "Invalid minout",
                )
            })
            it("zAsset does not exist (no approval for bPool)", async () => {
                const mZZZ = await zAssetMachine.loadBassetProxy("xZeno ZZZ", "mZZZ", 18)
    ...
    describe("testing asset management", () => {
        ...
        describe("reinvesting BAL", () => {
            ...
            beforeEach(async () => {
                mXYZ = await zAssetMachine.loadBassetProxy("xZeno XYZ", "mXYZ", 18)
                weth = await zAssetMachine.loadBassetProxy("xZeno ZZZ", "weth", 18)
