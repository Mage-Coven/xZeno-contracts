...
describe("Liquidator", () => {
    ...
    const redeployLiquidator = async () => {
        // Fake zUSD
        zUSD = await new MockZasset__factory(sa.default.signer).deploy("xZeno USD", "zUSD", 18, sa.fundManager.address, 100000000)
    ...
    context("performing basic system flow", async () => {
        describe("creating a new liquidation", () => {
            it("should set up all args", async () => {
                ...
                expect(liquidation.zAsset, "zAsset").eq(zUSD.address)
        ...
        describe("triggering a liquidation", () => {
            it("should sell COMP for bAsset and deposit to SavingsManager", async () => {
                const savingsManagerBalBefore = await zUSD.balanceOf(savings.address)
                ...
                expect(await zUSD.balanceOf(savings.address), "Savings Manager zUSD bal increased").gt(savingsManagerBalBefore)
            })
        })
    })
    ...
    context("triggering a liquidation for a zAsset", () => {
        beforeEach(async () => {
            await redeployLiquidator()
            await liquidator
                .connect(sa.governor.signer)
        ...
        it("should sell everything if the liquidator has less balance than tranche size", async () => {
            ...
            // 4,386.8 bAsset is then minted for zUSD which costs 2%
            // zUSD in Savings = 4,386.8 * (100 - 2) / 100 = 4,299.064
            const zAssetsExpected = simpleToExactAmount(4299064, 15)
            await expect(tx).to.emit(liquidator, "Liquidated").withArgs(compToken.address, zUSD.address, zAssetsExpected, bAsset.address)
            ...
            expect(s1.savingsManagerBal, "savings manager COMP bal after").to.eq(s0.savingsManagerBal.add(zAssetsExpected))
    ...
    context("triggering a liquidation for Feeder Pool", () => {
        beforeEach(async () => {
            ...
            await liquidator.connect(sa.governor.signer).createLiquidation(
                ...
                simpleToExactAmount(70, 18),
                ZERO_ADDRESS, // no zAsset. This is a Feeder Pool integration
                false,
            )
            await compIntegration.connect(sa.governor.signer).approveRewardToken()
        })
    ...
    context("Aave liquidation of zAsset", () => {
        before(async () => {
            await redeployLiquidator()
