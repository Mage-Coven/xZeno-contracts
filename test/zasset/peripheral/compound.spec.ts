...
describe("CompoundIntegration", () => {
    ...
    const runSetup = async (enableUSDTFee = false, simulateMint = false, skipInit = false) => {
        ...
        integrationDetails = await zAssetMachine.loadBassetsLocal(false, enableUSDTFee)
        ...
        compoundIntegration = await new CompoundIntegration__factory(sa.default.signer).deploy(nexus.address, zAsset.address, DEAD_ADDRESS)
        compoundIntegration = compoundIntegration.connect(sa.mockZasset.signer)

        if (!skipInit) {
            // set bAsset and cToken on the Compound Integration contract for the zAsset
            await Promise.all(
    ...
    describe("Compound constructor", async () => {
        ...
        it("should properly store deploy arguments", async () => {
            ...
            expect(await compoundIntegration.lpAddress(), "Liquidity provider (zAsset)").eq(zAsset.address)
        })
    ...
    describe("calling deposit", async () => {
        ...
        it("should deposit tokens to Compound", async () => {
            ...
            // Step 4. Simulate zAsset calling deposit on the integration contract from a mint
            // Transfer the zAsset some bAsset tokens. This would normally happen in a zAsset mint.
            await bAsset.transfer(compoundIntegration.address, amount)
            // zAsset calls deposit on the Compound integration contract
        ...
        it("should handle the fee calculations", async () => {
            ...
            // Step 1. simulate zAsset transferring bAsset tokens to integration contract as part of a mint
            await bAsset.transfer(compoundIntegration.address, amount)
            ...
            // Step 3. simulate zAsset calling deposit on the Integration contract
    ...
    describe("withdraw", () => {
        ...
        it("should withdraw tokens from Compound", async () => {
            ...
            // Step 2. simulate transfer of bAssets to integration contract in zAsset mint
            ...
            // Step 3. Simulate zAsset calling deposit
            ...
            // 4. Simulate zAsset calling withdraw on compound integration
        ...
        context("and specifying a minute amount of bAsset", () => {
            ...
            it("should withdraw 0 if the cToken amount is 0", async () => {
                ...
                const tx = compoundIntegration["withdraw(address,address,uint256,bool)"](zAsset.address, bAsset.address, amount, false)
