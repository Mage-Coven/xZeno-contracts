...
describe("AaveIntegration", async () => {
    ...
    const runSetup = async (enableUSDTFee = false, simulateMint = false, skipInit = false) => {
        ...
        integrationDetails = await zAssetMachine.loadBassetsLocal(true, enableUSDTFee, zAsset.address)
            ...
            if (simulateMint) {
                await Promise.all(
                    integrationDetails.aTokens.map(async ({ bAsset }) => {
                        ...
                        const b1 = await new MockERC20__factory(zAsset.signer).attach(bAsset)
                        ...
                        // Step 2. call deposit
                        return aaveIntegration.connect(zAsset.signer).deposit(bAsset, amountD.toString(), true)
                    }),
                )
            }
        }
    ...
    describe("AaveIntegration constructor", async () => {
        ...
        it("should properly store valid arguments", async () => {
            ...
            expect(await aaveIntegration.lpAddress()).eq(zAsset.address)
        ...
        it("should fail when zAsset address invalid", async () => {
            await expect(
        ...
        it("should fail if passed incorrect data", async () => {
            await expect(
                new AaveV2Integration__factory(sa.default.signer).deploy(nexus.address, zAsset.address, ZERO_ADDRESS, DEAD_ADDRESS),
    ...
    describe("calling deposit", async () => {
        ...
        it("should deposit tokens to Aave", async () => {
            ...
            // Step 2. call deposit
            const tx = aaveIntegration.connect(zAsset.signer).deposit(bAsset.address, amount.toString(), false)
        ...
        it("should handle the fee calculations", async () => {
            ...
            // Step 2. call deposit
            await aaveIntegration.connect(zAsset.signer).deposit(bAsset.address, receivedAmount.toString(), true)
        ...
        it("should fail if the bAsset is not supported", async () => {
            ...
            // Step 1. call deposit
            await expect(aaveIntegration.connect(zAsset.signer).deposit(bAsset.address, amount.toString(), false)).to.be.revertedWith(
        ...
        it("should fail if we try to deposit too much", async () => {
            ...
            await expect(aaveIntegration.connect(zAsset.signer).deposit(bAsset.address, amountHigh.toString(), false)).to.be.revertedWith(
                "ERC20: transfer amount exceeds balance",
            )
        })
        it("should fail with broken arguments", async () => {
            ...
            await expect(aaveIntegration.connect(zAsset.signer).deposit(ZERO_ADDRESS, amount.toString(), false)).to.be.revertedWith(
                "aToken does not exist",
            )
            // Fails with ZERO Amount
            await expect(aaveIntegration.connect(zAsset.signer).deposit(bAsset.address, "0", false)).to.be.revertedWith(
                "Must deposit something",
            )
            // Succeeds with Incorrect bool (defaults to false)
            await aaveIntegration.connect(zAsset.signer).deposit(bAsset.address, amount.toString(), undefined)
        ...
        it("should fail if lending pool does not exist", async () => {
            ...
            // Fails with ZERO Amount
            await expect(aaveIntegration.connect(zAsset.signer).deposit(bAsset.address, "1", false)).to.be.revertedWith(
    ...
    describe("withdraw", async () => {
        ...
        it("should fail if the bAsset is not supported", async () => {
            // Step 0. Choose tokens
            const bAsset = await zAssetMachine.loadBassetProxy("MK", "MK", 12)
            const amount = BN.from(10).pow(BN.from(12))
    ...
    describe("withdraw specific amount", async () => {
        describe("and the token does not have transfer fee", async () => {
            ...
            it("should allow withdrawal of X and give Y to the caller", async () => {
                ...
                // fail if called by non Bm or zAsset
                await expect(
                    aaveIntegration
                        .connect(sa.dummy1.signer)
    ...
    describe("withdrawRaw", async () => {
        ...
        it("should allow the zAsset or BM to withdraw a given bAsset", async () => {
            const bAsset = await new MockERC20__factory(sa.default.signer).attach(integrationDetails.aTokens[0].bAsset)
            const bAssetDecimals = await bAsset.decimals()
            const amount = simpleToExactAmount(5, bAssetDecimals)
            ...
            await aaveIntegration.connect(zAsset.signer).withdrawRaw(bAssetRecipient, bAsset.address, amount)
        ...
        it("should fail if there is no balance in a given asset", async () => {
            const bAsset = await new MockERC20__factory(sa.default.signer).attach(integrationDetails.aTokens[0].bAsset)
            await expect(
                aaveIntegration.connect(zAsset.signer).withdrawRaw(sa.dummy3.address, bAsset.address, BN.from(1)),
            ).to.be.revertedWith("ERC20: transfer amount exceeds balance")
        })
        it("should fail if specified a 0 amount", async () => {
            const bAsset = await new MockERC20__factory(sa.default.signer).attach(integrationDetails.aTokens[0].bAsset)
            await expect(
                aaveIntegration.connect(zAsset.signer).withdrawRaw(sa.dummy3.address, bAsset.address, BN.from(0)),
            ).to.be.revertedWith("Must withdraw something")
        })
    })
    ...
    describe("checkBalance", async () => {
        ...
        it("should fail if called with inactive token", async () => {
            const bAsset = await zAssetMachine.loadBassetProxy("MK", "MK1", 12)
