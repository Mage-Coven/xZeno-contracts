...
describe("Zasset - Swap", () => {
     ...
     * @param zAsset Zasset instance upon which to call swap
    ...
    const assertFailedSwap = async (
        ...
        const approval: BN = await zAssetMachine.approveZasset(inputBasset, zAsset, inputQuantity, sa.default.signer, quantitiesAreExact)
        ...
        await expect(
            zAsset.swap(inputBasset.address, outputAsset.address, approval, minOutputQuantityExact, recipient),
        ...
        if (callSwapOutput) {
            if (swapOutputRevertExpected) {
                await expect(
                    zAsset.getSwapOutput(inputBasset.address, outputAsset.address, approval),
                ...
                const output = await zAsset.getSwapOutput(inputBasset.address, outputAsset.address, approval)
    ...
    const assertSwap = async (
        ...
        const inputBassetBefore = await zAssetMachine.getBasset(details, inputBasset.address)
        const outputBassetBefore = await zAssetMachine.getBasset(details, outputAsset.address)
        const { surplus: surplusBefore } = await zAsset.data()

        // 2. Do the necessary approvals and make the calls
        const approval0: BN = await zAssetMachine.approveZasset(
            inputBasset,
            zAsset,
        ...
        const expectedOutputValue = await zAsset.getSwapOutput(inputBasset.address, outputAsset.address, approval0)

        // 3. Calculate expected responses
        const inputQuantityExact = approval0

        //     Expect to be used in cache
        const platformInteractionIn = await ZassetMachine.getPlatformInteraction(zAsset, "deposit", approval0, inputBassetBefore)
        const platformInteractionOut = await ZassetMachine.getPlatformInteraction(
            zAsset,
            "withdrawal",
            expectedOutputValue,
            outputBassetBefore,
        ...
        const swapTx = zAsset.swap(inputBasset.address, outputAsset.address, approval0, minOutputQuantity, recipient)

        // 4. Validate any basic events that should occur
        // Swapped event
        await expect(swapTx).to.emit(zAsset, "Swapped")
        ...
        await expect(swapTx, "Transfer event for input bAsset from sender to platform integration or zAsset")
            .to.emit(inputBasset, "Transfer")
            .withArgs(sender.address, inputBassetBefore.integrator ? inputBassetBefore.integratorAddr : zAsset.address, inputQuantityExact)
        await expect(swapTx, "Transfer event for output bAsset from platform integration or zAsset to recipient")
            .to.emit(outputAsset, "Transfer")
            .withArgs(outputBassetBefore.integrator ? outputBassetBefore.integratorAddr : zAsset.address, recipient, expectedOutputValue)
        await swapTx

        const inputIntegratorBalAfter = await inputBassetBefore.contract.balanceOf(
            inputBassetBefore.integrator ? inputBassetBefore.integratorAddr : zAsset.address,
        )
        ...
        const outputIntegratorBalAfter = await outputBassetBefore.contract.balanceOf(
            outputBassetBefore.integrator ? outputBassetBefore.integratorAddr : zAsset.address,
        )
        ...
        const inputBassetAfter = await zAsset.getBasset(inputBasset.address)
        ...
        const outputBassetAfter = await zAsset.getBasset(outputAsset.address)
    ...
    describe("swapping assets", () => {
        context("when within the invariant validator limits", () => {
            ...
            it("should swap with min qty same as qty less swap fee", async () => {
                ...
                const outputQty = await zAsset.getSwapOutput(inputBasset.address, outputBasset.address, inputQty)
            ...
            context("passing invalid arguments", async () => {
                ...
                it("should fail if identical bAssets", async () => {
                    ...
                    const input = bAssets[0]
                    const output = input
                    await assertFailedSwap(zAsset, input, output, 1, 0, "Invalid pair", sa.default.signer, sa.default.address, true, true)
                })
                ...
                it("should fail if sender doesn't give approval", async () => {
                    ...
                    expect(await input.allowance(sender.address, zAsset.address)).eq(0)
                    expect(await input.balanceOf(sender.address)).eq(10000)
                    await expect(
                        zAsset.connect(sender.signer).swap(input.address, bAssets[1].address, 5, 1, sa.default.address),
                ...
                it("should fail if *either* bAsset does not exist", async () => {
                    ...
                    await expect(zAsset.swap(fakeBasset, realBasset, 1, 0, recipient)).to.revertedWith(expectedReason)
                    await expect(zAsset.swap(realBasset, fakeBasset, 1, 0, recipient)).to.revertedWith(expectedReason)
                })
                it("should fail if *either* bAsset is ZERO", async () => {
                    ...
                    const expectedReason = "Invalid asset"
                    await expect(zAsset.swap(realBasset.address, fakeBasset, 1, 0, recipient)).to.revertedWith(expectedReason)
                    await expect(zAsset.getSwapOutput(realBasset.address, fakeBasset, 1)).to.revertedWith(expectedReason)
                    await expect(zAsset.swap(fakeBasset, realBasset.address, 1, 0, recipient)).to.revertedWith(expectedReason)
                    await expect(zAsset.getSwapOutput(fakeBasset, realBasset.address, 1)).to.revertedWith(expectedReason)
                })
                it("should fail using the zAsset as the input asset", async () => {
                    const { bAssets, zAsset } = details
                    await assertFailedSwap(zAsset, zAsset, bAssets[0], 1, 0, "Invalid asset", undefined, undefined, true, true)
                })
                it("should fail using an input asset not in the basket", async () => {
                    ...
                    await assertFailedSwap(zAsset, invalidBasset, bAssets[0], 1, 0, "Invalid asset", undefined, undefined, true, true)
                })
            })
            context("using bAssets with transfer fees", async () => {
                context("when no lending market", async () => {
                    ...
                    context("and input has xfer fee", () => {
                        it("should have lower input and proportionately lower output", async () => {
                            ...
                            //    Get basic before data on the swap assets
                            const inputBassetBefore = await zAsset.getBasset(inputBasset.address)
                            const outputBassetBefore = await zAsset.getBasset(outputAsset.address)

                            // 2. Do the necessary approvals and make the calls
                            const approval0: BN = await zAssetMachine.approveZasset(inputBasset, zAsset, swapQuantity, sa.default.signer)
                            await zAsset.swap(inputBasset.address, outputAsset.address, approval0, 0, recipient)
                ...
                context("when a lending market integration", async () => {
                    ...
                    context("and input has xfer fee", () => {
                        ...
                        it("should fail if the system doesn't know about the fee", async () => {
                            const { bAssets, zAsset } = details
                            await zAsset.connect(sa.governor.signer).setTransferFeesFlag(bAssets[3].address, false)
                            await assertFailedSwap(
                                zAsset,
                                bAssets[3],
                                bAssets[0],
                ...
                describe("when output has xfer fee", async () => {
                    ...
                    it("should continue to pay out", async () => {
                        ...
                        const outputAsset = bAssets[3]
                        const swapQuantity = 1
                        await zAsset.connect(sa.governor.signer).setTransferFeesFlag(outputAsset.address, false)
            ...
            context("with an affected bAsset", async () => {
                ...
                it("should fail if input basset has lost its peg", async () => {
                    ...
                    await zAsset.connect(sa.governor.signer).handlePegLoss(input.address, true)
                    const inputBasset = await zAsset.getBasset(input.address)
                    expect(inputBasset.personal.status).to.eq(BassetStatus.BrokenBelowPeg)
                    const outputBasset = await zAsset.getBasset(output.address)
                    expect(outputBasset.personal.status).to.eq(BassetStatus.Normal)
                    await assertFailedSwap(zAsset, input, output, 1, 0, "Unhealthy", undefined, undefined, true, false, 0.999003)
                })
                it("should fail if output basset has lost its peg", async () => {
                    ...
                    await zAsset.connect(sa.governor.signer).handlePegLoss(output.address, true)
                    const inputBasset = await zAsset.getBasset(input.address)
        ...
        context("with assets connected to lending markets", async () => {
            ...
            it("should deposit into and withdraw from the lending market", async () => {
                ...
                await zAssetMachine.seedWithWeightings(details, [1, 1, 1, 1])
