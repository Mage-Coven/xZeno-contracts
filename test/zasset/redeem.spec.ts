...
import { MockERC20, Zasset } from "types/generated"
...
describe("Zasset - Redeem", () => {
    ...
    const assertFailedBasicRedemption = async (
        ...
        zAssetRedeemQuantity: BN | number | string,
        ...
        const zAssetRedeemQuantityExact = quantitiesAreExact ? BN.from(zAssetRedeemQuantity) : simpleToExactAmount(zAssetRedeemQuantity, 18)
        ...
        await expect(
            zAsset.redeem(bAssetAddress, zAssetRedeemQuantityExact, minBassetOutputExact, recipient),
            `redeem tx should revert with "${expectedReason}"`,
        ).to.be.revertedWith(expectedReason)

        if (redeemOutputRevertExpected) {
            await expect(
                zAsset.getRedeemOutput(bAssetAddress, zAssetRedeemQuantityExact),
                `getRedeemOutput call should revert with "${expectedReason}"`,
            ).to.be.revertedWith(expectedReason)
        } else {
            const redeemedBassetQuantity = await zAsset.getRedeemOutput(bAssetAddress, zAssetRedeemQuantityExact)
            assertBNClosePercent(redeemedBassetQuantity, expectedBassetQuantityExact, "0.1", "getRedeemOutput call output")
        }
    }
    const assertFailedZassetRedemption = async (
        expectedReason: string,
        zAssetContract: Zasset,
        zAssetQuantity: BN | number | string,
        ...
        const zAssetQuantityExact = quantitiesAreExact ? BN.from(zAssetQuantity) : simpleToExactAmount(zAssetQuantity, 18)
        ...
        await expect(
            zAsset.redeemZasset(zAssetQuantityExact, minBassetQuantitiesExact, recipient),
            `redeem tx should revert with "${expectedReason}"`,
        ).to.be.revertedWith(expectedReason)
    }
    const assertFailedExactBassetsRedemption = async (
        ...
        maxZassetBurntQuantity: BN | number | string = 100,
        sender: Signer = sa.default.signer,
        recipient: string = sa.default.address,
        quantitiesAreExact = false,
        redeemOutputRevertExpected = true,
        expectedZassetQuantityExact: BN | number | string = BN.from(1),
    ): Promise<void> => {
        const zAsset = zAssetContract.connect(sender)
        ...
        const maxZassetBurntQuantityExact = quantitiesAreExact
            ? BN.from(maxZassetBurntQuantity)
            : simpleToExactAmount(maxZassetBurntQuantity, 18)

        await expect(
            zAsset.redeemExactBassets(bAssetAddresses, bAssetRedeemQuantitiesExact, maxZassetBurntQuantityExact, recipient),
            `redeem tx should revert with "${expectedReason}"`,
        ).to.be.revertedWith(expectedReason)

        if (redeemOutputRevertExpected) {
            await expect(
                zAsset.getRedeemExactBassetsOutput(bAssetAddresses, bAssetRedeemQuantitiesExact),
                `getRedeemExactBassetsOutput call should revert with "${expectedReason}"`,
            ).to.be.revertedWith(expectedReason)
        } else {
            const redeemedZassetQuantity = await zAsset.getRedeemExactBassetsOutput(bAssetAddresses, bAssetRedeemQuantitiesExact)
            assertBNClosePercent(
                redeemedZassetQuantity,
                BN.from(expectedZassetQuantityExact),
                "0.1",
    ...
    // redeem takes zAsset input and returns bAsset amount
    const assertBasicRedemption = async (
        md: ZassetDetails,
        bAsset: MockERC20,
        zAssetBurnQuantity: BN | number | string,
        ...
        // Get balances before
        const senderZassetBalBefore = await zAsset.balanceOf(sender.address)
        const zAssetSupplyBefore = await zAsset.totalSupply()
        ...
        const zAssetQuantityExact = quantitiesAreExact ? BN.from(zAssetBurnQuantity) : simpleToExactAmount(zAssetBurnQuantity, 18)
        const minBassetOutputExact = quantitiesAreExact ? BN.from(minBassetOutput) : simpleToExactAmount(minBassetOutput, bAssetDecimals)
        const { surplus: surplusBefore, swapFee: feeRate } = await zAsset.data()
        ...
        if (expectFee) {
            ...
            scaledFee = zAssetQuantityExact.mul(feeRate).div(fullScale)
        }

        const bAssetQuantityExact = await zAsset.getRedeemOutput(bAsset.address, zAssetQuantityExact)

        const platformInteraction = await ZassetMachine.getPlatformInteraction(zAsset, "withdrawal", bAssetQuantityExact, bAssetBefore)

        // Execute the redemption
        const tx = zAsset.redeem(bAsset.address, zAssetQuantityExact, minBassetOutputExact, recipient)
        ...
        await expect(tx)
            .to.emit(zAsset, "Redeemed")
            .withArgs(sender.address, recipient, zAssetQuantityExact, bAsset.address, bAssetQuantityExact, scaledFee)
        // - Withdraws from lending platform or zAsset
        ...
        // Transfer events
        await expect(tx, "Transfer event to burn the redeemed zAssets")
            .to.emit(zAsset, "Transfer")
            .withArgs(sender.address, ZERO_ADDRESS, zAssetQuantityExact)
        if (!hasTransferFee) {
            await expect(tx, "Transfer event for bAsset from platform integration or zAsset to recipient")
                .to.emit(bAsset, "Transfer")
                .withArgs(bAssetBefore.integrator ? bAssetBefore.integratorAddr : zAsset.address, recipient, bAssetQuantityExact)
        }
        await tx
        ...
        // Sender should have less zAsset
        const senderZassetBalAfter = await zAsset.balanceOf(sender.address)
        expect(senderZassetBalAfter, "senderZassetBalAfter").eq(senderZassetBalBefore.sub(zAssetQuantityExact))
        // Total zAsset supply should be less
        const zAssetSupplyAfter = await zAsset.totalSupply()
        expect(zAssetSupplyAfter, "zAssetSupplyAfter").eq(zAssetSupplyBefore.sub(zAssetQuantityExact))
        ...
        const bAssetAfter = await zAssetMachine.getBasset(details, bAsset.address)
    ...
    const assertExactBassetsRedemption = async (
        ...
        bAssets: MockERC20[],
        bAssetRedeemQuantities: (BN | number | string)[],
        maxZassetBurntQuantity: BN | number | string = 0,
        expectFee = true,
        recipient: string = sa.default.address,
        sender: Account = sa.default,
        ignoreHealthAssertions = false,
        ...
        const { surplus: surplusBefore } = await zAsset.data()

        const zAssetQuantityExact = await zAsset.getRedeemExactBassetsOutput(bAssetAddresses, bAssetRedeemQuantitiesExact)

        // Calculate redemption fee
        let scaledFee = BN.from(0)
        //    If there is a fee expected, then deduct it from output
        if (expectFee) {
            const { swapFee: feeRate } = await zAsset.data()
            expect(feeRate, "fee rate > 0").gt(BN.from(0))
            expect(feeRate, "fee rate < fullScale / 50").lt(fullScale.div(BN.from(50)))
            // fee = zAsset qty - (zAsset qty * feeRate / 1e18)
            scaledFee = zAssetQuantityExact.mul(feeRate).div(fullScale)
            expect(scaledFee, "scaled fee > 0").gt(BN.from(0))
        }

        // Execute the redemption
        const tx = zAsset.redeemExactBassets(bAssetAddresses, bAssetRedeemQuantitiesExact, maxZassetBurntQuantityExact, recipient)

        // Check the emitted events
        await expect(tx)
            .to.emit(zAsset, "RedeemedMulti")
            .withArgs(sender.address, recipient, zAssetQuantityExact, bAssetAddresses, bAssetRedeemQuantitiesExact, scaledFee)
        // Transfer events
        await expect(tx, "Transfer event to burn the redeemed zAssets")
            .to.emit(zAsset, "Transfer")
            .withArgs(sender.address, ZERO_ADDRESS, zAssetQuantityExact)
        ...
        await Promise.all(
            bAssets.map((bAsset, i) => {
                if (bAssetRedeemQuantitiesExact[i].gt(0)) {
                    return expect(tx, `Transfer event for bAsset[${i}] from platform integration or zAsset to recipient`)
                        .to.emit(bAsset, "Transfer")
                        .withArgs(
                            bAssetsBefore[i].integrator ? bAssetsBefore[i].integratorAddr : zAsset.address,
                            recipient,
        ...
        return zAssetQuantityExact
    }

    const assertZassetRedemption = async (
        md: ZassetDetails,
        zAssetQuantityGross: BN | number | string,
        minBassetQuantitiesNet: (BN | number | string)[] = [0, 0, 0, 0],
        expectFee = true,
        recipient: string = sa.default.address,
        sender: Account = sa.default,
        ...
        // Convert to exact quantities
        const zAssetQuantityExact = quantitiesAreExact ? BN.from(zAssetQuantityGross) : simpleToExactAmount(zAssetQuantityGross, 18)
        const minBassetQuantitiesExact = quantitiesAreExact
            ? minBassetQuantitiesNet.map((q) => BN.from(q))
            : minBassetQuantitiesNet.map((q, i) => simpleToExactAmount(q, bAssetsDecimals[i]))
        ...
        const zAssetSupplyBefore = await zAsset.totalSupply()
        const recipientBassetBalsBefore: BN[] = await Promise.all(bAssets.map((b) => b.balanceOf(recipient)))
        const { surplus: surplusBefore, redemptionFee: feeRate } = await zAsset.data()
        ...
        if (expectFee) {
            expect(feeRate, "fee rate > 0").gt(BN.from(0))
            expect(feeRate, "fee rate < fullScale / 50").lt(fullScale.div(BN.from(50)))
            // fee = zAsset qty * fee rate
            scaledFee = zAssetQuantityExact.mul(feeRate).div(fullScale)
            expect(scaledFee, "scaled fee > 0").gt(BN.from(0))
        }

        // Execute the redemption
        const tx = zAsset.redeemZasset(zAssetQuantityExact, minBassetQuantitiesExact, recipient)

        // (zAsset qty / 4) * (1 - redemption fee)
        const zAssetRedemptionAmountNet = zAssetQuantityExact.sub(zAssetQuantityExact.mul(feeRate).div(fullScale))
        const bAssetRedeemQuantitiesExact = bAssets.map((b, i) => {
            // netBassetRedemptionAmount = bAsset vault balance * zAsset quantity to be burnt / (total zAsset zAsset + surplus)
            const netBassetRedemptionAmount = BN.from(bAssetsBefore[i].vaultBalance)
                .mul(zAssetRedemptionAmountNet)
                .div(zAssetSupplyBefore.add(surplusBefore))
        ...
        context("when the weights are within the validator limit", () => {
            ...
            context("and the feeRate changes", async () => {
                ...
                it("should deduct the suitable fee", async () => {
                    ...
                    await zAsset.connect(sa.governor.signer).setFees(newSwapFee, newRedemptionFee)
                    // Calc zAsset burn amounts based on bAsset quantities
                    const zAssetQuantity = simpleToExactAmount(1)
                    const fee = zAssetQuantity.mul(newSwapFee).div(fullScale)
                    const zassetBalBefore = await zAsset.balanceOf(sa.default.address)
                    const { surplus: surplusBefore } = await zAsset.data()
                    // Run the redemption
                    await assertBasicRedemption(details, bAsset, BN.from(1), BN.from(0))
                    const zassetBalAfter = await zAsset.balanceOf(sa.default.address)
                    const { surplus: surplusAfter } = await zAsset.data()
                    // Assert balance increase
                    expect(zassetBalAfter).eq(zassetBalBefore.sub(zAssetQuantity))
                ...
                it("should deduct nothing if the fee is 0", async () => {
                    ...
                    await zAsset.connect(sa.governor.signer).setFees(newFee, newFee)
                    // Calc zAsset burn amounts based on bAsset quantities
                    const zAssetQuantity = simpleToExactAmount(1)
                    const zassetBalBefore = await zAsset.balanceOf(sa.default.address)
                    const { surplus: surplusBefore } = await zAsset.data()
            ...
            context("passing invalid arguments", async () => {
                ...
                it("should revert when bAsset is 0x0", async () => {
                    ...
                    await assertFailedBasicRedemption("Invalid asset", zAsset, ZERO_ADDRESS, 1)
                })
                it("should fail if the bAsset does not exist", async () => {
                    const { zAsset } = details
                    const invalidBasset = await zAssetMachine.loadBassetProxy("Wrapped ETH", "WETH", 18)
                    await assertFailedBasicRedemption("Invalid asset", zAsset, invalidBasset, 1)
                })
                it("should revert when 0 quantity", async () => {
                    const { bAssets, zAsset } = details
                    await assertFailedBasicRedemption("Qty==0", zAsset, bAssets[0], 0)
                })
                ...
                it("should fail if sender doesn't have zAsset balance", async () => {
                    const { bAssets, zAsset } = details
                    const sender = sa.dummy1
                    expect(await zAsset.balanceOf(sender.address)).eq(0)
                    await assertFailedBasicRedemption(
            ...
            context("with an affected bAsset", async () => {
                ...
                it("should fail if other bAssets in basket have broken peg", async () => {
                    const { bAssets, zAsset } = details
                    await assertBasketIsHealthy(zAssetMachine, details)
                    await zAsset.connect(sa.governor.signer).handlePegLoss(bAssets[0].address, false)
                    const abovePegBasset = await zAsset.getBasset(bAssets[0].address)
                    await zAsset.connect(sa.governor.signer).handlePegLoss(bAssets[1].address, true)
                    const belowPegBasset = await zAsset.getBasset(bAssets[1].address)
            ...
            context("performing multiple redemptions in a row", async () => {
                ...
                it("should redeem with single bAsset", async () => {
                    const { bAssets, zAsset } = details
                    const oneZasset = simpleToExactAmount(1, 18)
                    const zAssetSupplyBefore = await zAsset.totalSupply()
                    await Promise.all(
                        bAssets.map(async (b) => {
                            const bAssetDecimals = await b.decimals()
                            return zAsset.redeem(
                                b.address,
                    ...
                    const zAssetSupplyAfter = await zAsset.totalSupply()
                    expect(zAssetSupplyAfter).eq(zAssetSupplyBefore.sub(BN.from(bAssets.length).mul(oneZasset)))
                })
            })
            context("using bAssets with transfer fees", async () => {
                ...
                it("should handle tokens with transfer fees", async () => {
                    // // It should burn the full amount of zAsset, but the fees deducted mean the redeemer receives less
                    const { bAssets, zAsset } = details

                    const recipient = sa.dummy3
                    // 1.0 Assert bAsset has fee
                    const bAsset = bAssets[3]
                    const bAssetBefore = await zAsset.getBasset(bAsset.address)
                    ...
                    const expectedBassetQuantity = await zAsset.getRedeemOutput(bAsset.address, simpleToExactAmount(1))
                    ...
                    const totalSupplyAfter = await zAsset.totalSupply()
                ...
                it("should send less output to user if fee unexpected", async () => {
                    ...
                    const { swapFee } = await zAsset.data()
                    ...
                    const recipientBassetBalBefore = await bAsset.balanceOf(recipient.address)
                    await zAsset.redeem(bAsset.address, oneBasset, 0, recipient.address)
                    // 4.0 Total supply goes down, and recipient bAsset goes up slightly
                    const recipientBassetBalAfter = await bAsset.balanceOf(recipient.address)
                    // Assert that we redeemed gt 99% of the bAsset
        ...
        context("passing invalid arguments", async () => {
            ...
            context("when invalid bAssets", () => {
                ...
                it("should fail when empty bAsset and quantities arrays", async () => {
                    const { zAsset } = details
                    await assertFailedExactBassetsRedemption("Invalid array input", zAsset, [], [], 1)
                })
                it("should fail when empty bAsset and some quantities array input", async () => {
                    const { zAsset } = details
                    await assertFailedExactBassetsRedemption("Invalid array input", zAsset, [], [1, 2], 4)
                })
                it("should fail when some bAssets and empty quantities arrays", async () => {
                    const { zAsset } = details
                    await assertFailedExactBassetsRedemption("Invalid array input", zAsset, details.bAssets, [], 4)
                })
                it("should fail when bAssets to quantities array len do not match", async () => {
                    const { zAsset } = details
                    await assertFailedExactBassetsRedemption("Invalid array input", zAsset, details.bAssets, [1, 2], 4)
                })
                it("should fail when first bAsset is 0x0", async () => {
                    const { zAsset } = details
                    invalidBassets[0] = ZERO_ADDRESS
                    await assertFailedExactBassetsRedemption("Invalid asset", zAsset, invalidBassets, [1, 2, 3, 4], 11)
                })
            ...
            it("should revert when redeemed zAsset quantity > max zAsset quantity", async () => {
                const { bAssets, zAsset } = details
                await assertFailedExactBassetsRedemption(
                    "Redeem zAsset qty > max quantity",
                    zAsset,
                    bAssets,
                    ["1000000000000000000", "2000000", "3000000000000", "4000000000000000000"],
                    "10000000000000000000",
            ...
            context("when redeemed zAsset quantity just greater than max zAsset quantity", () => {
                it("should revert with high rounded number", async () => {
                    ...
                    await assertFailedExactBassetsRedemption(
                        ...
                        bAssets,
                        ["1000000000000000000", "2000000", "3000000000000", "4000000000000000000"],
                        // zAsset = 10 / (1 - 0.06 / 100) = 10.00600360216129677807
                        // 1 - 0.06 / 100 = 0.9994
    ...
    describe("redeeming zAssets for multiple proportional bAssets", () => {
        context("even bAsset weights", () => {
            ...
            it("should redeem with all different bAsset quantities", async () => {
                const recipient = details.managerLib.address
                await assertZassetRedemption(details, 10, [2, 2, 2, 2], true, recipient)
            })
            it("should redeem with bAsset minimums exactly equal", async () => {
                const recipient = details.managerLib.address
                await assertZassetRedemption(
                    details,
                    "10000000000000000000",
        ...
        context("uneven bAsset weights", () => {
            ...
            it("should redeem", async () => {
                const recipient = details.managerLib.address
                await assertZassetRedemption(details, 10, [0, 0, 5, 2], true, recipient)
            })
        })
        ...
        describe("passing invalid arguments", async () => {
            ...
            it("should revert when zAsset quantity is zero", async () => {
                const { bAssets, zAsset } = details
                await assertFailedZassetRedemption("Qty==0", zAsset, 0, [2, 2, 2, 2], bAssets)
            })
            it("should fail if recipient is 0x0", async () => {
                const { bAssets, zAsset } = details
                await assertFailedZassetRedemption("Invalid recipient", zAsset, 0, [2, 2, 2, 2], bAssets, undefined, ZERO_ADDRESS)
            })
        })
        describe("failures other than invalid arguments", () => {
            ...
            context("when a bAsset minimum is not reached", () => {
                const testData = [
                    ...
                    },
                    {
                        ...
                        zAssetQuantity: "10000000000000000000",
                ...
                testData.forEach((data) => {
                    it(`should revert when ${data.desc}`, async () => {
                        const { bAssets, zAsset } = details
                        await assertFailedZassetRedemption(
                            "bAsset qty < min qty",
                            zAsset,
                            data.zAssetQuantity || 10,
            ...
            it("should fail if sender doesn't have zAsset balance", async () => {
                ...
                const sender = sa.dummy1
                expect(await zAsset.balanceOf(sender.address)).eq(0)
                await assertFailedZassetRedemption("ERC20: burn amount exceeds balance", zAsset, 10, [2, 2, 2, 2], bAssets, sender.signer)
            })
            context("when a bAsset has broken its peg", () => {
                it("should fail if broken below peg", async () => {
                    ...
                    const newBasset = await zAsset.getBasset(bAsset.address)
                    expect(newBasset.personal.status, "bAsset broken below peg").to.eq(BassetStatus.BrokenBelowPeg)
                    await assertFailedZassetRedemption("In recol", zAsset, 10, [2, 2, 2, 2], bAssets)
                })
