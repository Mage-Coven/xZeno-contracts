...
import { ZassetDetails, ZassetMachine, StandardAccounts, FeederMachine, FeederDetails } from "@utils/machines"
...
interface ConfigRedeemAndUnwrap {
    ...
    router: ExposedZasset | FeederPool | MockERC20 // Router address = zAsset || feederPool
}
...
const getData = async (contract: SavingsContract, user: Account): Promise<Data> => {
    const zAsset = await (await new MockERC20__factory(user.signer)).attach(await contract.underlying())
    ...
    return {
        balances: {
            ...
            user: await zAsset.balanceOf(user.address),
            contract: await zAsset.balanceOf(contract.address),
        },
...
describe("SavingsContract", async () => {
    ...
    const createNewSavingsContract = async (): Promise<void> => {
        ...
        // Use a mock zAsset so we can dictate the interest generated
        zasset = await (await new MockZasset__factory(sa.default.signer)).deploy("MOCK", "MOCK", 18, sa.default.address, 1000000000)
        ...
        const impl = await savingsFactory.deploy(nexus.address, zasset.address, unwrapperContract.address)
    ...
    describe("constructor", async () => {
        it("should fail when zasset address is zero", async () => {
            await expect(savingsFactory.deploy(nexus.address, ZERO_ADDRESS, unwrapperContract.address)).to.be.revertedWith(
                "zAsset address is zero",
            )

            savingsContract = await savingsFactory.deploy(nexus.address, zasset.address, unwrapperContract.address)
        ...
        it("should fail when unwrapper address is zero", async () => {
            await expect(savingsFactory.deploy(nexus.address, zasset.address, ZERO_ADDRESS)).to.be.revertedWith("Unwrapper address is zero")
        })
        it("should succeed and set valid parameters", async () => {
            ...
            expect(zasset.address).to.equal(underlyingAddr)
    ...
    describe("depositing interest", async () => {
        ...
        beforeEach(async () => {
            ...
            await zasset.transfer(savingsManagerAccount.address, simpleToExactAmount(20, 18))
            await zasset.connect(savingsManagerAccount.signer).approve(savingsContract.address, simpleToExactAmount(20, 18))
        })
        afterEach(async () => {
        ...
        it("should deposit interest when some credits exist", async () => {
            ...
            await zasset.approve(savingsContract.address, deposit)
            await savingsContract.preDeposit(deposit, sa.default.address)

            const balanceBefore = await zasset.balanceOf(savingsContract.address)
    ...
    describe("depositing savings", async () => {
        context("using preDeposit", async () => {
            before(async () => {
                await createNewSavingsContract()
                await zasset.approve(savingsContract.address, simpleToExactAmount(1, 21))
                // This amount should not be collected
                await zasset.setAmountForCollectInterest(simpleToExactAmount(100, 18))
            })
            afterEach(async () => {
                const data = await getData(savingsContract, alice)
                expect(exchangeRateHolds(data), "Exchange rate must hold")
        ...
        async function validateDeposits(contractFn: ContractFnType) {
            ...
            describe(`using ${contractFn.name}`, async () => {
                ...
                it("should fail if the user has no balance", async () => {
                    // Approve first
                    await zasset.connect(sa.dummy1.signer).approve(savingsContract.address, simpleToExactAmount(1, 18))
                ...
                it("should deposit the zUSD and assign credits to the saver", async () => {
                    ...
                    await zasset.approve(savingsContract.address, depositAmount)
                    // 2. Deposit the zUSD
                ...
                context("when there is some interest to collect from the manager", async () => {
                    ...
                    it("should collect the interest and update the exchange rate before issuance", async () => {
                        ...
                        // Set up the zAsset with some interest
                        await zasset.setAmountForCollectInterest(interest)

                        await ethers.provider.send("evm_increaseTime", [ONE_DAY.toNumber()])
                        await ethers.provider.send("evm_mine", [])

                        // Bob deposits into the contract
                        await zasset.transfer(bob.address, deposit)
                        await zasset.connect(bob.signer).approve(savingsContract.address, deposit)
    ...
    describe("checking the view methods", () => {
        ...
        before(async () => {
            ...
            await zasset.setAmountForCollectInterest(simpleToExactAmount(10, 18))
    ...
    describe("minting", async () => {
        ...
        it("should mint the izUSD and assign credits to the saver", async () => {
            ...
            // 1. Approve the savings contract to spend zUSD
            await zasset.approve(savingsContract.address, assets)
            // 2. Deposit the zUSD
    ...
    describe("redeeming", async () => {
        async function validateRedeems(contractFn: ContractFnType) {
            ...
            describe(`scenario ${contractFn.name}`, async () => {
                ...
                describe(`using ${contractFn.name}`, async () => {
                    ...
                    // test the balance calcs here.. credit to zasset, and public calcs
                ...
                context("with a connector that surpasses limit", async () => {
                    ...
                    before(async () => {
                        ...
                        ).deploy(savingsContract.address, zasset.address)
        ...
        context("using redeem(uint256) (deprecated)", async () => {
            beforeEach(async () => {
                await createNewSavingsContract()
                await zasset.approve(savingsContract.address, simpleToExactAmount(10, 18))
                await savingsContract["depositSavings(uint256)"](simpleToExactAmount(1, 18))
            })
            it("should fail when input is zero", async () => {
            ...
            it("should withdraw the zUSD and burn the credits", async () => {
                ...
                const zUSDBefore = await zasset.balanceOf(sa.default.address)
                ...
                const zUSDAfter = await zasset.balanceOf(sa.default.address)
    ...
    describe("setting fraction", () => {
        before(async () => {
            await createNewSavingsContract()
            await zasset.approve(savingsContract.address, simpleToExactAmount(1, 18))
            await savingsContract.preDeposit(simpleToExactAmount(1, 18), sa.default.address)
        })
        it("fails if not called by governor", async () => {
    ...
    describe("setting connector", () => {
        ...
        beforeEach(async () => {
            await createNewSavingsContract()
            const connector = await connectorFactory.deploy(savingsContract.address, zasset.address)
        ...
        it("updates the connector address, moving assets to new connector", async () => {
            ...
            const newConnector = await connectorFactory.deploy(savingsContract.address, zasset.address)
    ...
    describe("poking", () => {
        ...
        context("with a lending market connector", () => {
            ...
            before(async () => {
                ...
                await zasset.transfer(connector.address, simpleToExactAmount(100, 18))
            ...
            it("should deposit to the connector if total supply increases", async () => {
                await zasset.approve(savingsContract.address, simpleToExactAmount(1, 20))
        ...
        context("with a vault connector", () => {
            ...
            before(async () => {
                await createNewSavingsContract()
                connector = await (await new MockVaultConnector__factory(sa.default.signer)).deploy(savingsContract.address, zasset.address)
                await zasset.transfer(connector.address, simpleToExactAmount(100, 18))
                await zasset.approve(savingsContract.address, simpleToExactAmount(1, 21))
                await savingsContract.preDeposit(deposit, alice.address)
            })
        ...
        context("with no connector", () => {
            ...
            it("simply updates the exchangeRate using the raw balance", async () => {
                ...
                await zasset.transfer(savingsContract.address, airdrop)
                const tx = savingsContract.poke()
                await expect(tx)
    ...
    describe("testing emergency stop", () => {
        ...
        before(async () => {
            ...
            await zasset.transfer(bob.address, simpleToExactAmount(100, 18))
            await zasset.approve(savingsContract.address, simpleToExactAmount(1, 21))
            await savingsContract.preDeposit(deposit, alice.address)
        ...
        it("should still allow deposits and withdrawals to work", async () => {
            await zasset.connect(bob.signer).approve(savingsContract.address, simpleToExactAmount(1, 21))
    ...
    ;[
        ...
        context(`performing multiple operations from multiple addresses in sequence - ${index}`, async () => {
            ...
            it("should give existing savers the benefit of the increased exchange rate", async () => {
                ...
                await zasset.transfer(saver2.address, saver2deposit)
                await zasset.transfer(saver3.address, saver3deposit)
                await zasset.transfer(saver4.address, saver4deposit)
                await zasset.connect(saver1.signer).approve(savingsContract.address, MAX_UINT256)
                await zasset.connect(saver2.signer).approve(savingsContract.address, MAX_UINT256)
                await zasset.connect(saver3.signer).approve(savingsContract.address, MAX_UINT256)
                await zasset.connect(saver4.signer).approve(savingsContract.address, MAX_UINT256)
                ...
                await zasset.setAmountForCollectInterest(interestToReceive1)
                ...
                await zasset.setAmountForCollectInterest(interestToReceive2)
                ...
                await zasset.setAmountForCollectInterest(interestToReceive3)
                ...
                await zasset.setAmountForCollectInterest(interestToReceive4)
 ...
 * @param {ZassetMachine} zAssetMachine mints 25 tokens for each bAsset
 ...
 * @return {*}  {Promise<ZassetDetails>}
 */
const deployZassets = async (
    zAssetMachine: ZassetMachine,
    seedBasket = true,
    useTransferFees = false,
    useLendingMarkets = false,
    weights: number[] = [25, 25, 25, 25],
): Promise<ZassetDetails> => {
    const details = await zAssetMachine.deployZasset(useLendingMarkets, useTransferFees)
    if (seedBasket) {
...
): Promise<FeederDetails> => feederMachine.deployFeeder(feederWeights, zAssetWeights, useLendingMarkets, useInterestValidator)

const createNewSavingsContract = async (
    zAssetMachine: ZassetMachine,
    details: ZassetDetails,
): Promise<{
    savingsContract: SavingsContract
    unwrapperContract: Unwrapper
    mockSavingsManager: MockSavingsManager
}> => {
    // Deploy Exposed Zassets Mocks
    const sa = zAssetMachine.sa
    const nexus = details.nexus
    const zasset = details.zAsset
...
const runSetup = async (
    ...
    mDetails: ZassetDetails
    fDetails: FeederDetails
    savingsContract: SavingsContract
}> => {
    // Deploy Exposed Zassets Mocks
    let mDetails: ZassetDetails
    let fDetails: FeederDetails
    const sa = zAssetMachine.sa

    if (feederMachine === null) {
        // Deploy Zasset +
        mDetails = await deployZassets(zAssetMachine)
    } else {
        // Deploy Zasset + Feeder pool
        fDetails = await deployFeeder(feederMachine)
        mDetails = fDetails.zAssetDetails
    }
    const zasset = mDetails.zAsset

    const { savingsContract, unwrapperContract, mockSavingsManager } = await createNewSavingsContract(zAssetMachine, mDetails)

    if (feederMachine) {
        // approve tokens for router
        const routers = [fDetails.pool.address, fDetails.pool.address]
        const tokens = [zasset.address, fDetails.fAsset.address]
    ...
    await zasset.transfer(savingsManagerAccountAddress, simpleToExactAmount(20, 18))
    await zasset.connect(savingsManagerAccountSigner).approve(savingsContract.address, simpleToExactAmount(20, 18))
    await zasset.connect(sa.default.signer).approve(savingsContract.address, simpleToExactAmount(20, 18))
...
describe("SavingsContract with Unwrapper", async () => {
    ...
    const validateAssetRedemption = async (config: ConfigRedeemAndUnwrap) => {
        ...
        // transfers tokens to sender
        expect(dataAfter.balances.user, "user zAsset balance unchanged").eq(dataBefore.balances.user)
        expect(dataAfter.balances.contract, "contract zAsset balance reduced").eq(dataBefore.balances.contract.sub(expectedWithdrawal))
    ...
    context("redeemAndUnwrap", async () => {
        ...
        context("zasset/izAsset to bAsset", async () => {
            ...
            before("Init contract", async () => {
                ...
                sa = zAssetMachine.sa
                alice = sa.default
                ;({ mDetails, fDetails, savingsContract, zasset } = await runSetup(zAssetMachine, null))
            ...
            it("should redeem zAsset to bAsset", async () => {
                // Given a N amount of zAsset (isCreditAmt = false)
            ...
            it("should redeem izAsset to bAsset", async () => {
                // Given a N amount of izAsset (isCreditAmt = true)
                config = {
                    ...config,
                    isCreditAmt: true,
                    amount: credits,
            ...
            context("fails", async () => {
                ...
                it("to redeem zAsset to zAsset as it is an 'Invalid asset'", async () => {
                    // Given n amount of zAsset
                    config = {
                        ...config,
                        amount: deposit,
                        isCreditAmt: false,
                ...
                it("to redeem izAsset to bAsset with wrong isBassetOut argument", async () => {
                    // Given a N amount of zAsset (isCreditAmt = true)
                    config = {
                        ...config,
                        isCreditAmt: true,
                        amount: credits,
                ...
                it("to redeem izAsset to zAsset with wrong isBassetOut argument", async () => {
                    // Given a N amount of zAsset (isCreditAmt = true)
                    config = {
                        ...config,
                        amount: credits,
                ...
                it("to redeem izAsset that exceeds balance", async () => {
                    // Given a N amount of izAsset (isCreditAmt = true)
                    const dataBefore = await getOutputData(savingsContract, config.beneficiary, config.output)
                ...
                it("to redeem zAsset that exceeds balance", async () => {
                    // Given a N amount of izAsset (isCreditAmt = false)
                    const dataBefore = await getOutputData(savingsContract, config.beneficiary, config.output)
                    config = {
                        ...config,
                        isCreditAmt: false,
                    ...
                    // When it redeems more zAsset that its balance
                    await expect(validateAssetRedemption(config)).to.be.revertedWith("ERC20: burn amount exceeds balance")
                })
            })
        })
        context("zasset/izAsset to fAsset", async () => {
            // Deploy saving contract and mock savings manager.
            before("init contract", async () => {
                const accounts = await ethers.getSigners()
                zAssetMachine = await new ZassetMachine().initAccounts(accounts)
                ...
                ;({ mDetails, fDetails, savingsContract, zasset } = await runSetup(zAssetMachine, feederMachine))
            ...
            it("should redeem zAsset to fAsset", async () => {
                // Given a N amount of zAsset (isCreditAmt = false)
                config = {
                    ...config,
                    isCreditAmt: false,
            ...
            it("should redeem izAsset to fAsset", async () => {
                // Given a N amount of izAsset (isCreditAmt = true)
                config = {
                    ...config,
                    isCreditAmt: true,
            ...
            context("fails", async () => {
                it("to redeem zAsset to fAsset with wrong isBassetOut argument", async () => {
                ...
                it("to redeem izAsset to fAsset with wrong isBassetOut argument", async () => {
                    // Given a N amount of zAsset (isCreditAmt = true)
                    config = {
                        ...config,
                        isCreditAmt: true,
                ...
                it("to redeem zAsset with wrong router", async () => {
                    // Given a N amount of izAsset (isCreditAmt = false)
                    config = {
                        ...config,
                        isCreditAmt: false,
