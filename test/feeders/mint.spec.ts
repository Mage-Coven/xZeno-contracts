...
import { FeederDetails, FeederMachine, ZassetMachine, StandardAccounts } from "@utils/machines"
...
describe("Feeder - Mint", () => {
    ...
    const runSetup = async (
        ...
        details = await feederMachine.deployFeeder(feederWeights, zAssetWeights, useLendingMarkets,
    ...
    const assertBasicMint = async (
        ...
        const minZassetQuantityExact = quantitiesAreExact
        ...
        const integratorBalBefore = await assetBefore.contract.balanceOf(
            assetBefore.integrator ? assetBefore.integratorAddr : assetBefore.feederPoolOrZassetContract.address,
        )
        ...
        expect(feederOutput, "zAssetOutput").to.eq(outputQuantityExact)

        const tx = pool.mint(inputAsset.address, assetQuantityExact, minZassetQuantityExact, recipient)
        ...
        // If not a main pool asset, so only a fAsset or zAsset
        // and expecting a deposit into lending markets
        if (!assetBefore.isMpAsset && platformInteraction.expectInteraction) {
            await expect(tx)
                .to.emit(fd.zAssetDetails.platform, "Deposit")
                .withArgs(
                    assetBefore.isMpAsset ? fd.zAsset.address : inputAsset.address,
                    assetBefore.isMpAsset ? fd.zAsset.address : assetBefore.pToken,
                    platformInteraction.amount,
                )
            // TODO this check is not working for zAssets or fAssets using a lending market
    ...
    describe("minting with a single asset", () => {
        context("when the basket is balanced", () => {
            context("passing invalid arguments", async () => {
                ...
                it("should fail to mint xZeno asset when sender doesn't give approval", async () => {
                    const { zAsset, pool } = details
                    const sender = sa.dummy2
                    await zAsset.transfer(sender.address, 10000)
                    expect(await zAsset.allowance(sender.address, pool.address)).eq(0)
                    expect(await zAsset.balanceOf(sender.address)).eq(10000)
                    await assertFailedMint(
                        "ERC20: transfer amount exceeds balance",
                        pool,
                        zAsset,
                ...
                it("should fail when the asset does not exist", async () => {
                    const newBasset = await feederMachine.zAssetMachine.loadBassetProxy("Mock", "MKK", 18, sa.default.address, 1000)
                ...
                context("when feeder pool is paused", () => {
                    ...
                    it("should fail to mint xZeno asset", async () => {
                        await assertFailedMint("Unhealthy", details.pool, details.zAsset, simpleToExactAmount(1), "999991742447046384")
                    })
            ...
            context("using bAssets with no transfer fees", async () => {
                context("reset before each", () => {
                    ...
                    it("should mint a single xZeno asset", async () => {
                        await assertBasicMint(details, details.zAsset, simpleToExactAmount(1), "999991742447046384")
                    })
                    ...
                    it("should mint a single main pool asset", async () => {
                        await assertBasicMint(details, details.zAssetDetails.bAssets[0], simpleToExactAmount(1), "999990257669407574")
                    })
                ...
                context("when a main pool asset has broken below peg", () => {
                    ...
                    before(async () => {
                        const { zAsset, zAssetDetails } = details
                        await zAsset.connect(sa.governor.signer).handlePegLoss(zAssetDetails.bAssets[0].address, true)
                        const newBasset = await zAsset.getBasset(zAssetDetails.bAssets[0].address)
                        expect(newBasset.personal.status).to.eq(BassetStatus.BrokenBelowPeg)
                    })
                    after(async () => {
                        const { zAsset, zAssetDetails } = details
                        await zAsset.connect(sa.governor.signer).negateIsolation(zAssetDetails.bAssets[0].address)
                        const newBasset = await zAsset.getBasset(zAssetDetails.bAssets[0].address)
                    ...
                    it("should fail to mint a main pool asset", async () => {
                        await assertFailedMint(
                            ...
                            details.pool,
                            details.zAssetDetails.bAssets[0],
                            simpleToExactAmount(1),
                            "999990257669407574",
                        )
                    })
        ...
        context("when the basket is initially 75% zAsset and 25% fAsset and then f is drastically scaled by the redemption price", () => {
            beforeEach(async () => {
                await runSetup(false, false, [75, 25], undefined, false, true)
            })
            it("set RP to 3 and mint with zAsset", async () => {
                const { redemptionPriceSnap } = details
                await redemptionPriceSnap.setRedemptionPriceSnap("3000000000000000000000000000")
                // fAsset is now worth 3 times as much as zAsset so total value in pool has increased by 50%. Now adding
                // one zAsset should return about two thirds of a pool token
                await assertBasicMint(details, details.zAsset, simpleToExactAmount(10), "6657940946958219439", "0")
            })
        ...
        context("when the basket is initially 33% zAsset and 67% fAsset and then RP drops to half", () => {
            beforeEach(async () => {
                await runSetup(false, false, [33, 67], undefined, false, true)
            })
            it("set RP to 0.5 and mint with zAsset", async () => {
                const { redemptionPriceSnap } = details
                await redemptionPriceSnap.setRedemptionPriceSnap("500000000000000000000000000")
                // zAsset is worth double fAsset and TVL has dropped by a third, so expect 1 zAsset to return 1 / (2/3)
                await assertBasicMint(details, details.zAsset, simpleToExactAmount(10), "15025149508501158974", "0")
            })
        ...
        context("when initially liquidity is split 50/50", () => {
            ...
            it("set RP to make zAsset overweight", async () => {
                const { redemptionPriceSnap } = details
                await redemptionPriceSnap.setRedemptionPriceSnap("240000000000000000000000000")
                // zAsset is now over 80% and mint should fail
                await assertFailedMint("Exceeds weight limits", details.pool, details.zAsset, simpleToExactAmount(10))

                await redemptionPriceSnap.setRedemptionPriceSnap("300000000000000000000000000")
                // zAsset is now just ok and small mint should succeed
                await assertBasicMint(details, details.zAsset, simpleToExactAmount(10), "15326009964632835267", "0")
            })
        ...
        context("when the basket is 75% zAsset, 25% fAsset", () => {
            beforeEach(async () => {
                await runSetup(false, false, [75, 25])
            })
            it("should mint zAsset to just under max weight", async () => {
                await assertBasicMint(details, details.zAsset, simpleToExactAmount(4), "3983329877010604242")
            })
            it("should fail mint zAsset over max weight", async () => {
                await assertFailedMint("Exceeds weight limits", details.pool, details.zAsset, simpleToExactAmount(40))
            })
            context("zAsset is overweight", () => {
                beforeEach(async () => {
                    // set new weight limits to 10% and 90% so the zAsset is overweight
                    await details.pool.connect(sa.governor.signer).setWeightLimits(simpleToExactAmount(30, 16), simpleToExactAmount(70, 16))
                })
                it("should fail to mint the overweight zAsset", async () => {
                    await assertFailedMint("Exceeds weight limits", details.pool, details.zAsset, simpleToExactAmount(1, 7))
                })
                it("should fail to mint fAsset if zAsset is still overweight", async () => {
                    await assertFailedMint("Exceeds weight limits", details.pool, details.zAsset, simpleToExactAmount(1))
                })
                it("should mint fAsset so zAsset is underweight", async () => {
        ...
        context("when using lending markets", async () => {
            context("deposit to lending markets", () => {
                ...
                it("should mint a single xZeno asset", async () => {
                    await assertBasicMint(details, details.zAsset, simpleToExactAmount(500), "498673496146378809664")
                })
                it("should mint a single feeder asset", async () => {
                    await assertBasicMint(details, details.fAsset, simpleToExactAmount(500), "498673496146378809664")
                })
                it("should mint a single main pool asset", async () => {
                    await assertBasicMint(details, details.zAssetDetails.bAssets[0], simpleToExactAmount(500), "498339345218159685928")
                })
            })
        })
    })
    describe("minting with multiple bAssets", () => {
            ...
            context("passing invalid arguments", async () => {
                ...
                context("with incorrect bAsset array", async () => {
                    ...
                    it("should multi mint a single main pool asset", async () => {
                        await assertFailedMintMulti("Invalid asset", details.pool, [details.zAssetDetails.bAssets[0]], [1])
                    })
                })
                context("when all quantities are zero", () => {
                    it("should fail to mint fAsset and zAsset", async () => {
                        await assertFailedMintMulti("Zero zAsset quantity", details.pool, details.bAssets, [0, 0], 0)
                    })
                    it("should fail to mint feeder asset", async () => {
                        await assertFailedMintMulti("Zero zAsset quantity", details.pool, [details.fAsset], [0], 0)
                    })
                    it("should fail to mint xZeno asset", async () => {
                        await assertFailedMintMulti("Zero zAsset quantity", details.pool, [details.zAsset], [0], 0)
                    })
                })
                ...
                it("should fail to mint xZeno asset when sender doesn't give approval", async () => {
                    const { bAssets, zAsset, pool } = details
                    const sender = sa.dummy2
                    await zAsset.transfer(sender.address, 10000)
                    expect(await zAsset.allowance(sender.address, pool.address)).eq(0)
                    expect(await zAsset.balanceOf(sender.address)).eq(10000)
            ...
            context("using bAssets with no transfer fees", async () => {
                context("reset before each", () => {
                    ...
                    it("should multi mint a single xZeno asset", async () => {
                        await assertMintMulti(details, [details.zAsset], [simpleToExactAmount(1)], "999991742447046384", 0)
                    })
        ...
        context("when the initial ratio is 50/50 and the redemption price is modified", () => {
            ...
            it("set RP to make zAsset overweight", async () => {
                ...
                await redemptionPriceSnap.setRedemptionPriceSnap("300000000000000000000000000")
                // zAsset is now just ok and small mint should succeed
                await assertMintMulti(details, [details.zAsset], [simpleToExactAmount(10)], "15326009964632835267")
            })
            it("set RP to make fAsset overweight", async () => {
                const { redemptionPriceSnap } = details
                await redemptionPriceSnap.setRedemptionPriceSnap("4500000000000000000000000000")
        ...
        context("when the basket is 21% zAsset, 79% fAsset", () => {
            beforeEach(async () => {
                await runSetup(false, false, [21, 79])
            })
            it("should fail to multi mint the smallest unit of fAsset", async () => {
                await assertFailedMintMulti("Zero zAsset quantity", details.pool, [details.fAsset], [1], 0, 0, true)
            })
            ...
            context("fAsset is overweight", () => {
                ...
                it("should fail to multi mint zAsset if fAsset is still overweight", async () => {
                    await assertFailedMintMulti(
                        "Exceeds weight limits",
                        details.pool,
                        [details.fAsset],
                ...
                it("should mint zAsset so fAsset is underweight", async () => {
                    await assertMintMulti(details, [details.zAsset], [simpleToExactAmount(30)], "30146363829134129434")
                })
            })
        })
    })
