                ...
                testData.mints.forEach((testMint) => {
                    if (testMint.hardLimitError) {
                        ...
                        it(`${(count += 1)} deposit ${testMint.bAssetQty.toString()} bAssets with index ${
                            ...
                            const zAssetQty = await exposedFeeder.computeMint(
                                reserves,
                                testMint.bAssetIndex,
            ...
            describe(`reserves: ${testData.reserve0}, ${testData.reserve1}`, () => {
                testData.mints.forEach((testMint) => {
                    ...
                    it(`${(count += 1)} deposit ${qtys} bAssets`, async () => {
                        const zAssetQty = await exposedFeeder.computeMintMulti(reserves, [0, 1], qtys, localConfig)
                        expect(zAssetQty).eq(cv(testMint.expectedQty))
                    })
                })
            })
        ...
        testRedeemData.forEach((testData) => {
            ...
            describe(`reserves: ${testData.reserve0}, ${testData.reserve1}`, () => {
                testData.redeems.forEach((testRedeem) => {
                    ...
                    const netInput = cv(testRedeem.zAssetQty).mul(fullScale.sub(redemptionFeeRate)).div(fullScale)
                ...
                testData.redeems.forEach((testRedeem) => {
                    ...
                    if (testRedeem.insufficientLiquidityError) {
                        ...
                        it(`${(count += 1)} redeem ${qtys} bAssets`, async () => {
                            const zAssetQty = await exposedFeeder.computeRedeemExact(reserves, [0, 1], qtys, localConfig)
                            assertBNClose(applyFee(zAssetQty), cv(testRedeem.zAssetQty), tolerance)
                        })
                    }
                })
            })
        ...
        testRedeemData.forEach((testData) => {
            ...
            describe(`reserves: ${testData.reserve0}, ${testData.reserve1}`, () => {
                ...
                before(async () => {
                    ...
                    const zAssetDetails = await zAssetMachine.deployZasset(false, false)
                    await zAssetMachine.seedWithWeightings(zAssetDetails, [25000000, 25000000, 25000000, 25000000])
                    zAssetBassets = zAssetDetails.bAssets
                    const bBtc = await zAssetMachine.loadBassetProxy("Binance BTC", "bBTC", 18)
                    bAssets = [zAssetDetails.zAsset as MockERC20, bBtc]
                ...
                testData.redeems.forEach((testRedeem) => {
                    ...
                    if ("insufficientLiquidityError" in testRedeem) {
                        it(`${(count += 1)} throws throw insufficient liquidity error when redeeming ${
                            ...
                            await expect(feederPool.redeemProportionately(cv(testRedeem.zAssetQty), qtys, recipient)).to.be.revertedWith(
                                "VM Exception",
                            )
                        })
                        ...
                        it(`${(count += 1)} redeem ${testRedeem.zAssetQty} zAssets for proportionate bAssets`, async () => {
                            await feederPool.redeemProportionately(cv(testRedeem.zAssetQty), qtys, recipient)
                        })
                    }
                })
            })
