...
import {
    ...
    ZassetLogic,
    ZassetManager,
    ZassetLogic__factory,
    ZassetManager__factory,
    MockERC20__factory,
...
export interface ZassetDetails {
    zAsset?: ExposedZasset
    ...
    logicLib?: ZassetLogic
    managerLib?: ZassetManager
    wrappedManagerLib?: ZassetManager
    nexus?: MockNexus
}

export class ZassetMachine {
    public sa: StandardAccounts

    public async initAccounts(accounts: Signer[]): Promise<ZassetMachine> {
        this.sa = await new StandardAccounts().initAccounts(accounts)
        return this
    }

    // 3 bAssets, custom reserve
    public async deployLite(a = 135): Promise<ZassetDetails> {
        const bAssets = await Promise.all([0, 1, 2].map((i) => this.loadBassetProxy(`${i}BASSET`, `${i}BASSET`, 18)))

        // 2. Invariant Validator
        const logicLib = await new ZassetLogic__factory(this.sa.default.signer).deploy()

        // 3. Invariant Validator
        const ManagerFactory = await new ZassetManager__factory(this.sa.default.signer)
        ...
        const zAssetFactoryLibs = {
            "contracts/zasset/ZassetLogic.sol:ZassetLogic": logicLib.address,
            "contracts/zasset/ZassetManager.sol:ZassetManager": managerLib.address,
        }
        const ZassetFactory = new ExposedZasset__factory(zAssetFactoryLibs, this.sa.default.signer)
        const impl = (await ZassetFactory.deploy(nexus.address, simpleToExactAmount(5, 13))) as ExposedZasset
        ...
        const zAsset = await new AssetProxy__factory(this.sa.default.signer).deploy(impl.address, DEAD_ADDRESS, data)

        return {
            zAsset: (await ZassetFactory.attach(zAsset.address)) as ExposedZasset,
            bAssets,
            logicLib: logicLib as ZassetLogic,
            nexus,
        }
    }

    public async deployZasset(useLendingMarkets = false, useTransferFees = false, a = 100): Promise<ZassetDetails> {
        // 1. Bassets
        ...
        const ZassetFactory = new ExposedZasset__factory(zAssetFactoryLibs, this.sa.default.signer)
        const impl = (await ZassetFactory.deploy(nexus.address, simpleToExactAmount(5, 13))) as Zasset
        ...
        if (useLendingMarkets) {
            await new MockPlatformIntegration__factory(this.sa.default.signer).attach(integrationAddress).addWhitelist([zAsset.address])
        }
        return {
            zAsset: (await ZassetFactory.attach(zAsset.address)) as ExposedZasset,
            ...
            logicLib,
            managerLib,
            wrappedManagerLib: await ManagerFactory.attach(zAsset.address),
            nexus,
        }
    }

    /**
     * @dev Seeds the zAsset basket with custom weightings
     * @param md Zasset details object containing all deployed contracts
     * @param weights Whole numbers of zAsset to mint for each given bAsset
     */
    public async seedWithWeightings(md: ZassetDetails, weights: Array<BN | number>, inputIsInBaseUnits = false): Promise<void> {
        const { zAsset, bAssets } = md
        const approvals = await Promise.all(
            bAssets.map((b, i) => this.approveZasset(b, zAsset, weights[i], this.sa.default.signer, inputIsInBaseUnits)),
        )
    ...
    // /**
    //  * @dev Deploys an zAsset with default parameters, modelled on original zUSD
    //  * @return Interface will all deployed information
    //  */
    // public async deployZasset(enableUSDTFee = false, useOldAave = false): Promise<ZassetDetails> {
    //     const md: ZassetDetails = {};
    ...
    //     // 2.4.2. Deploy zUSD
    //     // Deploy implementation
    //     const d_zUSD = await c_Zasset.new();
    ...
    //     );
    //     md.zAsset = await c_Zasset.at(d_zUSDProxy.address);
    ...
    public async loadBassetProxy(
        ...
        const zAssetProxy = await AssetProxyFactory.deploy(mockInitializableToken.address, this.sa.governor.address, data)
        const zAsset = MockERC20__factory.connect(zAssetProxy.address, this.sa.default.signer)
        return zAsset
    }
     ...
     *   - add new A token to the zAsset platform integration
     * @param bAssets
     * @returns
     */
    ...
    // /**
    //  * @dev Deploy a Zasset via the Manager then:
    //  *      1. Mint with optimal weightings
    //  */
    // public async deployZassetAndSeedBasket(
    //     enableUSDTFee = false,
    //     initialSupply = 100,
    // ): Promise<ZassetDetails> {
    //     const zAssetDetails = await this.deployZasset(enableUSDTFee);

    //     // Mint initialSupply with shared weightings
    //     const basketDetails = await this.getBassetsInZasset(zAssetDetails);
    ...
    //     // Approve bAssets
    //     await Promise.all(
    //         zAssetDetails.bAssets.map((b, i) =>
    //             b.approve(zAssetDetails.zAsset.address, mintAmounts[i].muln(2), {
    //                 from: this.system.sa.default,
    //             }),
    //         ),
    //     );

    //     await zAssetDetails.zAsset.mintMulti(
    ...
    //     return zAssetDetails;
    // }

    public async getBassetsInZasset(zAssetDetails: ZassetDetails): Promise<Basset[]> {
        const [personal, data] = await zAssetDetails.zAsset.getBassets()
    ...
    public async getBasset(zAssetDetails: ZassetDetails, bAssetAddress: string): Promise<Basset> {
        const bAsset = await zAssetDetails.zAsset.getBasset(bAssetAddress)
    ...
    public async getBasketComposition(zAssetDetails: ZassetDetails): Promise<BasketComposition> {
        // raw bAsset data
        const bAssets = await this.getBassetsInZasset(zAssetDetails)

        const [failed, undergoingRecol] = await zAssetDetails.zAsset.getBasket()
        // total supply of zAsset
        const supply = await zAssetDetails.zAsset.totalSupply()
        const { surplus } = await zAssetDetails.zAsset.data()
        // get actual balance of each bAsset
        const rawBalances = await Promise.all(
            bAssets.map((b) =>
                b.integrator ? b.contract.balanceOf(b.integrator.address) : b.contract.balanceOf(zAssetDetails.zAsset.address),
            ),
        )
     ...
     * @param zAsset Zasset that gets permission to spend
     * @param fullZassetUnits Whole number or fraction to approve
     * @param sender Set if needed, else fall back to default
     * @param inputIsBaseUnits Override the scaling up to ZassetQ
     */
    public async approveZasset(
        bAsset: MockERC20,
        zAsset: Zasset | ExposedZasset | MockERC20 | AssetProxy,
        fullZassetUnits: number | BN | string,
        sender: Signer = this.sa.default.signer,
        inputIsBaseUnits = false,
    ): Promise<BN> {
        const bAssetDecimals = await bAsset.decimals()
        const approvalAmount: BN = inputIsBaseUnits ? BN.from(fullZassetUnits) : simpleToExactAmount(fullZassetUnits, bAssetDecimals)
        await bAsset.connect(sender).approve(zAsset.address, approvalAmount)
        return approvalAmount
    }

    public async approveZassetMulti(
        bAssets: Array<MockERC20>,
        zAsset: Zasset | ExposedZasset,
        fullZassetUnits: number,
        sender: Signer,
    ): Promise<Array<BN>> {
        const result = Promise.all(bAssets.map((b) => this.approveZasset(b, zAsset, fullZassetUnits, sender)))
        return result
    }
    ...
    public static async getPlatformInteraction(
        ...
        const integratorBalBefore = await bAsset.contract.balanceOf(bAsset.integrator ? bAsset.integratorAddr : zAsset.address)
        if (hasIntegrator) {
            return {
        ...
        const totalSupply = await zAsset.totalSupply()
        const { cacheSize, surplus } = await zAsset.data()
