import { ZassetMachine, ZassetDetails } from "@utils/machines"
...
export const assertBnGte = (actual: BN, comparison: BN): void => {
    assert.ok(actual.gte(comparison), `Number must be GTE comparitor, got: ${actual.toString()}; comparitor: ${comparison.toString()}`)
}
...
export const assertBNSlightlyGT = (
    ...
    assert.ok(mustBeGreater ? actual.gt(equator) : actual.gte(equator), `Actual value should be greater than the expected value ${str}`)
    assert.ok(
 ...
 *  @param mustBeGreater Fail if the operands are equal
 */
export const assertBNSlightlyGTPercent = (actual: BN, equator: BN, maxPercentIncrease = "0.1", mustBeGreater = false): void => {
    ...
    assert.ok(mustBeGreater ? actual.gt(equator) : actual.gte(equator), `Actual value should be greater than the expected value`)
...
export const assertBasketIsHealthy = async (machine: ZassetMachine, md: ZassetDetails): Promise<void> => {
    // Read full basket composition
    const composition = await machine.getBasketComposition(md)
    // Assert sum of bAssets in vault storage is gte to total supply of zAsset
    ...
    // not failed
    expect(composition.failed, "zAsset not failed").to.eq(false)
    // prepareForgeBasset works
    // Potentially wrap in mock and check event
    await md.zAsset["getBasset(address)"](md.bAssets[0].address)
}
