...
import { ZassetHelpers } from "../shared/ZassetHelpers.sol";
 ...
 *          Derived by xZeno and adapted for the needs of an zAsset, as described in MIP-7 (http://mips.xzeno.org/MIPS/mip-7)
 *          Calculates and validates the result of Zasset operations with respect to the invariant.
...
library ZassetLogic {
    ...
    ****************************************/
    ...
    /**
     * @notice Transfers token in, updates internal balances and computes the zAsset output
     * @param _data                 Zasset storage state
     * @param _config               Core config for use in the invariant validator
     * @param _input                Data on the bAsset to deposit for the minted zAsset.
     * @param _inputQuantity        Quantity in input token units.
     * @param _minOutputQuantity    Minimum zAsset quantity to be minted. This protects against slippage.
     * @return mintOutput           Quantity of zAsset minted from the deposited bAsset.
     */
    function mint(
        ZassetData storage _data,
    ...
    /**
     * @notice Transfers tokens in, updates internal balances and computes the zAsset output.
     * Only fAsset & zAsset are supported in this path.
     * @param _data                 Zasset storage state
     * @param _config               Core config for use in the invariant validator
     * @param _indices              Non-duplicate addresses of the bAssets to deposit for the minted zAsset.
    ...
    function mintMulti(
        ...
        // Transfer the Bassets to the integrator, update storage and calc ZassetQ
        ...
        require(mintOutput >= _minOutputQuantity, "Mint quantity < min qty");
        require(mintOutput > 0, "Zero zAsset quantity");
    }
    ...
    ****************************************/
    ...
    /**
     * @notice Swaps two assets - either internally between fAsset<>zAsset, or between fAsset<>mpAsset by
     * first routing through the zAsset pool.
     * @param _data              Zasset storage state
     * @param _config            Core config for use in the invariant validator
     ...
     * @return scaledFee          Fee paid, in zAsset terms
     */
    function swap(
        ZassetData storage _data,
        InvariantConfig calldata _config,
    ...
    ****************************************/
    ...
    /**
     * @notice Burns a specified quantity of the senders zAsset in return for a bAsset. The output amount is derived
     * from the invariant. Supports redemption into either the fAsset, zAsset or assets in the zAsset basket.
     * @param _data              Zasset storage state
     * @param _config            Core config for use in the invariant validator
     * @param _output            Data on bAsset to withdraw
     * @param _inputQuantity   Quantity of zAsset to burn
     * @param _minOutputQuantity Minimum bAsset quantity to receive for the burnt zAsset. This protects against slippage.
     * @param _recipient         Address to transfer the withdrawn bAssets to.
     * @return bAssetQuantity    Quanity of bAsset units received for the burnt zAsset
     * @return scaledFee          Fee paid, in zAsset terms
     */
    function redeem(
        ZassetData storage _data,
        ...
        // Apply fees, burn zAsset and return bAsset to recipient
        _data.surplus += scaledFee;
     ...
     * balance levels and desired zAsset quantity. Burns the zAsset as payment. Only fAsset & zAsset are supported in this path.
     * @param _data                 Zasset storage state
     * @param _config               Core config for use in the invariant validator
     * @param _inputQuantity        Quantity of zAsset to redeem
     * @param _minOutputQuantities  Min units of output to receive
     * @param _recipient            Address to credit the withdrawn bAssets
     * @return scaledFee            Fee collected in zAsset terms
        ...
        )
    {
        ...
        // Calculate zAsset redemption quantities
        ...
        // Calc cache and total zAsset circulating
        uint256 maxCache = _getCacheDetails(_data, _config.supply - _inputQuantity + scaledFee);
     ...
     *      relative zAsset quantity from the sender. Only fAsset & zAsset (0,1) are supported in this path.
     * @param _data                 Zasset storage state
     * @param _config               Core config for use in the invariant validator
     ...
     * @return zAssetQuantity      Quantity of zAsset units to burn as payment
     * @return fee             Fee collected, in zAsset terms
     */
    function redeemExactBassets(
        ...
        address _recipient
    ) external returns (uint256 zAssetQuantity, uint256 fee) {
        // Load bAsset data from storage to memory
        BassetData[] memory cachedBassetData = _data.bAssetData;

        (zAssetQuantity, fee) = computeRedeemExact(
            cachedBassetData,
            _indices,
            _outputQuantities,
            _config,
        ...
        require(zAssetQuantity <= _maxZassetQuantity, "Redeem zAsset qty > max quantity");
        // Apply fees, burn zAsset and return bAsset to recipient
        _data.surplus += fee;
        // Transfer the Bassets to the recipient and count fees
        uint256 maxCache = _getCacheDetails(_data, _config.supply - zAssetQuantity + fee);
    ...
    ****************************************/
    ...
    function _depositTokens(
        ...
        if (_bAsset.integrator == address(0)) {
            (uint256 received, ) = ZassetHelpers.transferReturnBalance(
                msg.sender,
                address(this),
                _bAsset.addr,
                _quantity
        ...
        uint256 cacheBal;
        (quantityDeposited, cacheBal) = ZassetHelpers.transferReturnBalance(
            msg.sender,
            _bAsset.integrator,
            _bAsset.addr,
            _quantity
    ...
    /**
     * @dev Gets the max cache size, given the supply of zAsset
     * @return maxCache    Max units of any given bAsset that should be held in the cache
     */
    function _getCacheDetails(ZassetData storage _data, uint256 _supply)
        internal
        view
    ...
    ****************************************/
    ...
    /**
     * @notice Compute the amount of zAsset received for minting
     ...
     * @return mintAmount   Quantity of zAssets minted
     */
     ...
     * @return scaledSwapFee          Swap fee collected, in zAsset terms
     */
     ...
     * redeeming `quantity` amount of zAsset.
     * @param _bAssets              Array of all bAsset Data
     * @param _o                    Index of output bAsset
     * @param _grossZassetQuantity  Net amount of zAsset to redeem
     * @param _config               Generalised invariantConfig stored externally
    ...
    function computeRedeem(
        ...
        uint256 k0 = _invariant(x, sum, _config.a);
        uint256 redemption;
        (redemption, scaledFee) = _getFee(_grossZassetQuantity, _config, _feeRate, k0);
    ...
    function _getFee(
        ...
        redemption = _grossZassetQuantity;
        if (_config.supply > _k0) {
    ...
    /**
     * @notice Compute the amount of zAsset required to redeem
     * a given selection of bAssets.
     * @param _bAssets          Array of all bAsset Data
     * @param _indices          Indexes of output bAssets
     * @param _rawOutputs       Desired raw bAsset outputs
     * @param _config           Generalised invariantConfig stored externally
     * @return grossZasset      Amount of zAsset required to redeem bAssets
    ...
    function computeRedeemExact(
        ...
        // 5. Total zAsset is the difference between values
        uint256 redeemed = (_config.supply * (k0 - k1)) / k0;
        require(redeemed > 1e6, "Must redeem > 1e6 units");
        grossZasset = redeemed.divPrecisely(1e18 - _feeRate);
        fee = grossZasset - redeemed;
        grossZasset += 1;
        if (_config.supply > k0) {
            grossZasset = ((grossZasset * 1e18) / (1e18 - _config.recolFee));
        }
    }

    /**
     * @notice Gets the price of the zAsset, and invariant value k
     * @param _bAssets  Array of all bAsset Data
     * @param _config   Generalised InvariantConfig stored externally
     * @return price    Price of an zAsset
    ...
    ****************************************/
     ...
     * @param _config       Generalised InvariantConfig stored externally
     * @return mintAmount   Amount of value added to invariant, in zAsset terms
     */
