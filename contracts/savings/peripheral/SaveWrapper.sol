...
contract SaveWrapper is ImmutableModule {
    ...
    /**
     * @dev 0. Simply saves an zAsset and then into the vault
     * @param _zAsset   zAsset address
     * @param _save     Save address
     * @param _vault    Boosted Savings Vault address
     * @param _amount   Units of zAsset to deposit to savings
     */
    function saveAndStake(
        ...
        _saveAndStake(_zAsset, _save, _vault, _amount, true, address(0));
    }
    ...
    function saveAndStake(
        ...
        _saveAndStake(_zAsset, _save, _vault, _amount, true, _referrer);
    }

    /**
     * @dev 1. Mints an zAsset and then deposits to Save/Savings Vault
     * @param _zAsset       zAsset address
     * @param _bAsset       bAsset address
     * @param _save         Save address
     * @param _vault        Boosted Savings Vault address
     * @param _amount       Amount of bAsset to mint with
     * @param _minOut       Min amount of zAsset to get back
     * @param _stake        Add the izAsset to the Boosted Savings Vault?
     */
    function saveViaMint(
        ...
        _saveViaMint(_zAsset, _save, _vault, _bAsset, _amount, _minOut, _stake, address(0));
    }
    ...
    function saveViaMint(
        ...
        _saveViaMint(_zAsset, _save, _vault, _bAsset, _amount, _minOut, _stake, _referrer);
    }

    /**
     * @dev 2. Swaps fAsset for zAsset and then deposits to Save/Savings Vault
     * @param _zAsset             zAsset address
     * @param _save               Save address
     * @param _vault              Boosted Savings Vault address
     ...
     * @param _minOutputQuantity  Min amount of zAsset to be swapped and deposited
     * @param _stake              Deposit the izAsset in the Savings Vault?
     */
    ...
    /**
     * @dev 3. Buys a bAsset on Uniswap with ETH, then mints izAsset via zAsset,
     ...
     * @param _minOutXZeno  Min amount of zAsset to receive
     * @param _stake          Add the izAsset to the Savings Vault?
     */
    ...
    /**
     * @dev Gets estimated zAsset output from a WETH > bAsset > zAsset trade
     * @param _zAsset       zAsset address
     * @param _uniswap      Uniswap router address
    ...
    function estimate_saveViaUniswapETH(
        ...
        require(_zAsset != address(0), "Invalid zAsset");
        require(_uniswap != address(0), "Invalid uniswap");

        uint256 estimatedBasset = _getAmountOut(_uniswap, _ethAmount, _path);
        return IZasset(_zAsset).getMintOutput(_path[_path.length - 1], estimatedBasset);
    }
     ...
     * @param _save       Save address
     * @param _vault      Boosted vault address
     * @param _amount     Amount of zAsset to deposit
     * @param _stake      Add the izAsset to the Savings Vault?
    ...
    function _saveViaMint(
        ...
        // 2. Mint
        uint256 zassetsMinted = IZasset(_zAsset).mint(_bAsset, _amount, _minOut, address(this));

        // 3. Mint izAsset and optionally stake in vault
        _depositAndStake(_save, _vault, zassetsMinted, _stake, _referrer);
    }
    ...
    function _saveViaSwap(
        ...
        // 1. Swap the fAsset for zAsset with the feeder pool
        uint256 zAssetQuantity = IFeederPool(_feeder).swap(
            _fAsset,
            _zAsset,
            _fAssetQuantity,
            _minOutputQuantity,
        ...
        // 2. Deposit the zAsset into Save and optionally stake in the vault
        _depositAndStake(_save, _vault, zAssetQuantity, _stake, _referrer);
    }
    ...
    function _saveViaUniswapETH(
        ...
        // 2. Purchase zAsset
        uint256 zassetsMinted = IZasset(_zAsset).mint(
    ...
    /**
     * @dev Approve zAsset and bAssets, Feeder Pools and fAssets, and Save/vault
     */
    function approve(
        address _zAsset,
        ...
        _approve(_zAsset, _save);
        _approve(_save, _vault);
        _approve(_bAssets, _zAsset);
