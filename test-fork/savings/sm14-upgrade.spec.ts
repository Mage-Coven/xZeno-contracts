...
context("StakedToken deployments and vault upgrades", () => {
    ...
    const snapData = async (_savingsManager: SavingsManager, zAssets: string[]): Promise<Data> => ({
        savingsContracts: await Promise.all(zAssets.map((m) => savingsManager.savingsContracts(m))),
        revenueRecipients: await Promise.all(zAssets.map((m) => savingsManager.revenueRecipients(m))),
        nexus: await savingsManager.nexus(),
        lastPeriodStart: await Promise.all(zAssets.map((m) => savingsManager.lastPeriodStart(m))),
        lastCollection: await Promise.all(zAssets.map((m) => savingsManager.lastCollection(m))),
        periodYield: await Promise.all(zAssets.map((m) => savingsManager.periodYield(m))),
        liqStream: await Promise.all(zAssets.map((m) => savingsManager.liqStream(m))),
        yieldStream: await Promise.all(zAssets.map((m) => savingsManager.yieldStream(m))),
        lastBatchCollected: await Promise.all(zAssets.map((m) => savingsManager.lastBatchCollected(m))),
    })
    ...
    context("1. Deploying", () => {
        ...
        it("deploys new contract", async () => {
            zusd = resolveAddress("zUSD", Chain.mainnet, "address")
            zbtc = resolveAddress("zBTC", Chain.mainnet, "address")
        ...
        it("checks the config matches up", async () => {
            ...
            const oldConfig = await snapData(oldSavingsManager, [zusd, zbtc])
            const newConfig = await snapData(savingsManager, [zusd, zbtc])
        ...
        it("Reapprove zAssets to SavingsManager", async () => {
            await liquidator.reApproveLiquidation(USDC.integrator) // COMP for zUSD
            await liquidator.reApproveLiquidation(USDT.integrator) // AAVE for zUSD
            await liquidator.reApproveLiquidation(WBTC.integrator) // AAVE for zBTC
        })
    })
    context("2. Beta tests", () => {
        it("collects & streams interest from both zAssets", async () => {
            await savingsManager.collectAndStreamInterest(zusd)
            await savingsManager.collectAndStreamInterest(zbtc)
        })
        it("allows save deposits", async () => {
            const save = resolveAddress("zUSD", Chain.mainnet, "savings")
            await ERC20__factory.connect(zusd, zusdWhale).approve(save, simpleToExactAmount(1000))
            await SavingsContract__factory.connect(save, zusdWhale)["depositSavings(uint256)"](simpleToExactAmount(1000))
        })
        it("distributed unallocated interest", async () => {
            await savingsManager.distributeUnallocatedInterest(zusd)
            await savingsManager.distributeUnallocatedInterest(zbtc)
        })
