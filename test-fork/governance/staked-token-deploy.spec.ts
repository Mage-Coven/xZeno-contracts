...
interface StakedTokenDeployment {
    ...
    stakedTokenZENO: StakedTokenZENO
...
// 1. Deploy core stkZENO, mBPT variant & QuestManager
// 2. Gov TX's
...
//     1. 32.5k for stkZENO, 20k for stkMBPT
...
context("StakedToken deployments and vault upgrades", () => {
    ...
    context("1. Deploying", () => {
        it.skip("deploys the contracts", async () => {
            ...
            let stakedTokenAddresses: StakedTokenDeployAddresses = {
                ...
                stakedToken: getChainAddress("StakedTokenZENO", Chain.mainnet),
                stakedTokenImpl: "",
                initData: "",
            }
                ...
                stakedTokenAddresses = await deployStakingToken(
                    {
                        ...
                        stakedTokenSymbol: "ZENO",
                        cooldown: ONE_WEEK.mul(3).toNumber(),
                        name: "Staked Token ZENO",
                        symbol: "stkZENO",
                    },
                    deployer,
                    hre,
            ...
            deployedContracts = {
                ...
                stakedTokenZENO: StakedTokenZENO__factory.connect(stakedTokenAddresses.stakedToken, deployer.signer),
                ...
                zeno: IERC20__factory.connect(resolveAddress("ZENO"), deployer.signer),
        ...
        it("upgrade proxies", async () => {
            deployedContracts = {
                ...
                stakedTokenZENO: StakedTokenZENO__factory.connect(resolveAddress("StakedTokenZENO"), deployer.signer),
            ...
            await deployedContracts.proxyAdmin
                .connect(governor)
                .upgrade(deployedContracts.stakedTokenZENO.address, "0xCE1dA6331B07037E7e3a7B713f7Fd2C4F0A17A96")
            await deployedContracts.proxyAdmin
                .connect(governor)
                .upgrade(deployedContracts.stakedTokenBPT.address, "0x24083ee3919421E7477FF7A0b4b550EcE0e5d87E")
        })
        it("verifies stakedTokenZENO config", async () => {
            const config = await snapConfig(deployedContracts.stakedTokenZENO)
            expect(config.name, "name").eq("Staked ZENO")
            expect(config.symbol, "symbol").eq("stkZENO")
            ...
            expect(config.stakingToken, "staking token symbol").eq(resolveAddress("ZENO"))
            expect(config.rewardToken, "reward token symbol").eq(resolveAddress("ZENO"))
    ...
    context("2. Sending Gov Tx's", () => {
        it.skip("adds StakingTokens to BoostDirector and QuestManager", async () => {
            // 1. BoostDirector
            await deployedContracts.boostDirector.connect(governor).addStakedToken(deployedContracts.stakedTokenZENO.address)
            await deployedContracts.boostDirector.connect(governor).addStakedToken(deployedContracts.stakedTokenBPT.address)

            // 2. QuestManager
            await deployedContracts.questManager.connect(governor).addStakedToken(deployedContracts.stakedTokenZENO.address)
        ...
        it("adds small amount of rewards to both reward contracts", async () => {
            ...
            await rewardsDistributor
                .connect(fundManager)
                .distributeRewards(
                    [deployedContracts.stakedTokenZENO.address, deployedContracts.stakedTokenBPT.address],
                    [simpleToExactAmount(1), simpleToExactAmount(1)],
                )
        })
        it.skip("whitelists Badger voterproxy", async () => {
            await deployedContracts.stakedTokenZENO.connect(governor).whitelistWrapper(xZenoVoterProxy)
        })
    })
    context("3. Vault upgrades", () => {
        ...
        it("should verify the vault upgrades have executed successfully and all behaviour is in tact", async () => {
            ...
            const rewardTokenAddress = resolveAddress("ZENO")
    ...
    // deployer transfers 50k ZENO to Staker1 & 100k to Staker2
    // staker1 stakes in both
    // staker2 stakes in ZENO
    context("4. Beta testing", () => {
        let staker1signer: Signer
        const staker1bpt = simpleToExactAmount(2000)
        let staker2signer: Signer
        before(async () => {
            // tops up users with ZENO
            await deployedContracts.zeno.transfer(staker1, simpleToExactAmount(50000))
            await deployedContracts.zeno.transfer(staker2, simpleToExactAmount(100000))
        ...
        it("allows staker 1 staking on StakedTokenZENO", async () => {
            const startingRawBal = BN.from("200016043320105818350") // a bit over 200 ZENO
            const stakeAmount = simpleToExactAmount(50000)
            const balBefore = await snapshotUserStakingData(
                deployedContracts.stakedTokenZENO,
                deployedContracts.questManager,
                deployedContracts.zeno,
                staker1,
                true,
            )
            ...
            await deployedContracts.zeno.connect(staker1signer).approve(deployedContracts.stakedTokenZENO.address, stakeAmount)
            await deployedContracts.stakedTokenZENO.connect(staker1signer)["stake(uint256)"](stakeAmount)
        ...
        it("allows staker 2 staking on StakedTokenZENO", async () => {
            await deployedContracts.zeno
                .connect(staker2signer)
                .approve(deployedContracts.stakedTokenZENO.address, simpleToExactAmount(100000))
            await deployedContracts.stakedTokenZENO.connect(staker2signer)["stake(uint256)"](simpleToExactAmount(100000))
        })
        ...
        const calcBoost = (raw: BN, vZENO: BN, priceCoefficient = simpleToExactAmount(1)): BN => {
            ...
            let rhs = floor.add(flooredZENO.mul(coeff).div(10).mul(simpleToExactAmount(1)).div(simpleToExactAmount(denom)))
        ...
        it("should allow users to enter cooldown and withdraw", async () => {
            ...
            await deployedContracts.stakedTokenZENO.connect(staker2signer).startCooldown(simpleToExactAmount(50000))
            ...
            await deployedContracts.stakedTokenZENO.connect(staker2signer).withdraw(simpleToExactAmount(40000), staker2, false, true)
        ...
        it("should allow immediate upgrades of staking tokens", async () => {
            ...
            expect(await deployedContracts.proxyAdmin.getProxyAdmin(deployedContracts.stakedTokenZENO.address)).eq(
            ...
            await deployedContracts.proxyAdmin.changeProxyAdmin(deployedContracts.stakedTokenZENO.address, DEAD_ADDRESS)
        })
    ...
    context("5. Finalise", () => {
        ...
        it("should expire the old staking contract", async () => {
            ...
            const zenoVaultAddress = resolveAddress("ZENO", Chain.mainnet, "vault")
            const votingLockup = IncentivisedVotingLockup__factory.connect(zenoVaultAddress, governor)
    ...
    context("6. Test Badger migration", () => {
        it("should allow badger to stake in new contract", async () => {
            ...
            // await expect(voterProxy.changeLockAddress(deployedContracts.stakedTokenZENO.address)).to.be.revertedWith("Active lockup")
            // 2. Exit from old (exit)
            await voterProxy.connect(governor).exitLock()
            // 3. Update address ()
            await voterProxy.changeLockAddress(deployedContracts.stakedTokenZENO.address)
            // 4. fail when calling harvestZeno or increaseLockAmount/length
            await expect(voterProxy.connect(governor).harvestZeno()).to.be.revertedWith("Nothing to increase")
